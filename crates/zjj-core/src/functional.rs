    ...
    F: Fn(U, T) -> Result<U>,
{
    items
        .iter()
        .try_fold(init, |acc, item| f(acc, item.clone()))
}
...
mod tests {
    ...
    #[test]
    fn test_group_by() {
        let items = im::vector![("a", 1), ("b", 2), ("a", 3), ("b", 4)];
        let grouped = group_by(&items, |(key, _)| *key);

        assert_eq!(grouped.len(), 2);
        assert_eq!(grouped.get("a").map(im::Vector::len).unwrap_or(0), 2);
        assert_eq!(grouped.get("b").map(im::Vector::len).unwrap_or(0), 2);
    }

    #[test]
    fn test_partition() {
        let items = im::vector![1, 2, 3, 4, 5, 6];
        let (even, odd) = partition(&items, |x| x % 2 == 0);

        assert_eq!(even, im::vector![2, 4, 6]);
        assert_eq!(odd, im::vector![1, 3, 5]);
    }

    #[test]
    fn test_fold_result() {
        let items = im::vector![1, 2, 3, 4, 5];
    ...
    #[test]
    fn test_map_result() {
        let items = im::vector![1, 2, 3];
        let result = map_result(&items, |x| Ok(x * 2));
        assert_eq!(result.unwrap_or_default(), im::vector![2, 4, 6]);
    }
    ...
    fn test_filter_result() {
        ...
        assert_eq!(result.unwrap_or_default(), im::vector![2, 4]);
    }
}
