...
use crate::domain::{session::BranchState, SessionName};
...
impl SessionOutputBuilder {
    ...
    pub fn build(self) -> Result<SessionOutput, BuilderError> {
        ...
        Ok(SessionOutput {
            ...
            state: output_state,
            workspace_path,
            branch: self.branch.map(|b| b.to_string()),
            created_at: now,
            updated_at: updated,
        })
    }
...
/// Builder for `QueueEntry` with fluent API
///
/// # Required Fields
/// - `id`: Queue entry ID
...
///
/// # Optional Fields
/// - `status`: Entry status (defaults to Pending)
/// - `bead`: Attached bead
...
pub struct QueueEntryBuilder {
    ...
    bead: Option<BeadAttachment>,
    agent: Option<AgentAssignment>,
    created_at: Option<DateTime<Utc>>,
    updated_at: Option<DateTime<Utc>>,
}
...
impl QueueEntryBuilder {
    ...
    /// Set the entry ID (required)
    #[must_use]
    pub const fn id(mut self, id: QueueEntryId) -> Self {
        self.id = Some(id);
        self
    }
    ...
    /// Set the session name (required)
    #[must_use]
    pub fn session(mut self, session: SessionName) -> Self {
    ...
    /// Set the entry status (optional)
    #[must_use]
    pub const fn status(mut self, status: QueueEntryStatus) -> Self {
        self.status = Some(status);
        self
    }

    /// Set the attached bead (optional)
    #[must_use]
    pub fn bead(mut self, bead: BeadAttachment) -> Self {
        self.bead = Some(bead);
        self
    }

    /// Set the assigned agent (optional)
    #[must_use]
    pub fn agent(mut self, agent: AgentAssignment) -> Self {
    ...
    /// Set the update timestamp (optional)
    #[must_use]
    pub const fn updated_at(mut self, updated_at: DateTime<Utc>) -> Self {
        self.updated_at = Some(updated_at);
        self
    ...
    pub fn build(self) -> Result<QueueEntry, BuilderError> {
        ...
        let priority = self
            .priority
            .ok_or(BuilderError::MissingRequired { field: "priority" })?;
        ...
        Ok(QueueEntry {
            ...
            session,
            priority,
            status: self.status.unwrap_or(QueueEntryStatus::Pending),
            bead: self.bead.unwrap_or(BeadAttachment::None),
            agent: self.agent.unwrap_or(AgentAssignment::Unassigned),
            created_at: now,
            updated_at: updated,
        })
    }
...
/// Builder for [Stack] with entry collection
///
/// # Required Fields
/// - `name`: Stack/session name
/// - `base_ref`: Git base reference
///
/// # Optional Fields
/// - `entries`: Stack entries (can be added incrementally)
/// - `updated_at`: Update timestamp (defaults to now)
#[derive(Debug, Clone)]
pub struct StackBuilder {
    ...
    // Optional fields
    entries: Vec<StackEntryData>,
    updated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone)]
struct StackEntryData {
    session: SessionName,
    workspace: PathBuf,
...
impl StackBuilder {
    ...
    /// Set the base reference (required)
    #[must_use]
    pub fn base_ref(mut self, base_ref: BaseRef) -> Self {
    ...
    /// # Errors
    ///
    /// Returns `BuilderError::Overflow` if the entry count exceeds `u32::MAX`.
    pub fn with_entry(
        mut self,
        session: SessionName,
        workspace: PathBuf,
        ...
        let _order = u32::try_from(self.entries.len()).map_err(|_| BuilderError::Overflow {
            field: "entries",
            capacity: u32::MAX as usize,
        })?;

        self.entries.push(StackEntryData {
            session,
            workspace,
            status,
            bead,
...
/// Builder for [Train] with step collection
///
/// # Required Fields
/// - `id`: Train identifier
/// - `name`: Train/session name
///
/// # Optional Fields
/// - `steps`: Train steps (can be added incrementally)
/// - `status`: Train status (defaults to Pending)
/// - `created_at`: Creation timestamp (defaults to now)
/// - `updated_at`: Update timestamp (defaults to now)
#[derive(Debug, Clone)]
...
impl TrainBuilder {
    ...
    /// Set the train ID (required)
    #[must_use]
    pub fn id(mut self, id: TrainId) -> Self {
        self.id = Some(id);
        self
    }

    /// Set the train name (required)
    #[must_use]
    pub fn name(mut self, name: SessionName) -> Self {
        self.name = Some(name);
        self
    ...
    /// Add a step with an error to the train
    ///
    /// # Errors
    ///
    /// Returns `BuilderError::Overflow` if the step count exceeds `u32::MAX`.
    pub fn with_step_error(
        mut self,
        session: SessionName,
        action: TrainAction,
        status: TrainStepStatus,
    ...
    /// Set the train status (optional)
    #[must_use]
    pub const fn status(mut self, status: TrainStatus) -> Self {
        self.status = Some(status);
        self
    }
    ...
    pub fn build(self) -> Result<Train, BuilderError> {
        ...
        Ok(Train {
            ...
            name,
            steps,
            status: self.status.unwrap_or(TrainStatus::Pending),
            created_at: now,
            updated_at: updated,
        })
    }
...
mod tests {
    use super::*;
    use crate::output::domain_types::BeadId;

    // Helper to create valid session name for tests
    fn session_name(name: &str) -> SessionName {
        SessionName::parse(name).expect("valid session name")
    }

    // Helper to create valid bead ID for tests
    fn bead_id(id: &str) -> BeadId {
        BeadId::parse(id).expect("valid bead id")
    }
    ...
    #[test]
    fn test_queue_entry_builder_complete() {
        let id = QueueEntryId::new(1).expect("valid id");
        let name = session_name("test-session");

        let result = QueueEntryBuilder::new()
            .id(id)
            .session(name)
            .priority(5)
            .build();
        ...
        assert!(matches!(entry.status, QueueEntryStatus::Pending));
    }

    #[test]
    fn test_train_builder_with_steps() {
        let id = TrainId::new("train-1".to_string()).expect("valid id");
        let name = session_name("test-session");

        let result = TrainBuilder::new()
            .id(id)
            .name(name)
            .with_step(
                session_name("step1"),
