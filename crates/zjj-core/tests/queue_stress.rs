...
async fn stress_concurrent_claim_with_massive_contention() -> Result<()> {
    ...
    println!("Stress test results: {} successful claims, {} failed attempts", claims, failures);
    ...
    assert_eq!(claims, 5, "Exactly 5 agents should successfully claim entries");
    
    // Verify no duplicate workspace processing
    let all_entries = queue.list(None).await?;
    let processing_count = all_entries.iter()
        .filter(|e| e.status == QueueStatus::Processing)
        .count();
    
    assert_eq!(processing_count, 5, "Exactly 5 entries should be in processing state");
...
async fn stress_deadlock_prevention_under_timeout() -> Result<()> {
    ...
    assert!(entry2.is_none(), "Second agent should not claim while lock held");
    ...
    assert!(entry3.is_some(), "Second agent should claim after lock release");
...
async fn stress_concurrent_mark_operations() -> Result<()> {
    ...
    // Add 20 entries
    for i in 0..20 {
        queue.add(&format!("ws-{i}"), None, 5, Some("agent-initial")).await?;
    }
