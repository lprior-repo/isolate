...
async fn adv_duplicate_workspace_add_fails() -> Result<()> {
    ...
    assert!(
        err.to_string().contains("already in the queue"),
...
async fn adv_remove_nonexistent_returns_false() -> Result<()> {
    ...
    assert!(
        !removed,
        "Removing nonexistent workspace should return false"
    );
...
async fn adv_lock_timeout_with_active_work() -> Result<()> {
    ...
    .await?;
    short_timeout_queue
        .add("ws-short-timeout", None, 5, None)
        .await?;
...
async fn adv_position_of_nonexistent() -> Result<()> {
    ...
    assert!(
        position.is_none(),
        "Position of nonexistent workspace should be None"
    );
...
async fn adv_empty_queue_next() -> Result<()> {
    ...
    assert!(
        next_with_lock.is_none(),
        "Next with lock on empty queue should return None"
    );
...
async fn adv_stats_across_concurrent_ops() -> Result<()> {
    ...
    assert!(
        stats.completed + stats.pending == 20,
