//! Property-based tests for CLI structure invariants using proptest.
//!
//! These tests define the contract that the CLI structure must satisfy.
//! All tests verify invariants for the `zjj <object> <action>` pattern.
//!
//! # Invariants tested:
...
//!
//! Run with: cargo test --package zjj-core --test cli_properties
...
/// Exit codes following zjj conventions
...
/// Property: Terminal statuses are rejected for SessionOutput::new
///
/// INVARIANT: Completed and Failed statuses cannot be used when creating new sessions
#[test]
fn prop_terminal_statuses_rejected() {
    ...
    assert!(
        matches!(
            completed_result,
            Err(OutputLineError::TerminalStatus(SessionStatus::Completed))
        ),
        "Completed status should be rejected with TerminalStatus error"
    );

    // Failed status should be rejected
    ...
    assert!(
        matches!(
            failed_result,
            Err(OutputLineError::TerminalStatus(SessionStatus::Failed))
        ),
        "Failed status should be rejected with TerminalStatus error"
    );
}
