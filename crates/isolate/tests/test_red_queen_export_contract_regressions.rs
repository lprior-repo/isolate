...
fn bdd_export_help_json_returns_single_envelope() {
    ...
    let result = harness.isolate(&["export", "--help", "--json"]);
    assert!(
        result.success,
    ...
    assert_eq!(
        ...
        Some("isolate://cli-display-response/v1")
    );
    assert_eq!(
...
fn bdd_export_parser_rejects_duplicate_json_flag_with_single_json_error() {
    ...
    let result = harness.isolate(&["export", "--json", "--json"]);
    assert!(
        !result.success,
...
fn bdd_export_missing_session_fails_non_zero_in_human_and_json_modes() {
    ...
    let human = harness.isolate(&["export", "missing-session"]);
    assert!(
        !human.success,
    ...
    let json_result = harness.isolate(&["export", "missing-session", "--json"]);
    assert!(
        !json_result.success,
...
fn bdd_contract_export_and_import_exist_with_runtime_schemas() {
    ...
    let export_contract = harness.isolate(&["contract", "export", "--json"]);
    assert!(
    ...
    assert_eq!(
        export_payload
            .get("output_schema")
            .and_then(serde_json::Value::as_str),
        Some("isolate://export-response/v1")
    );

    let import_contract = harness.isolate(&["contract", "import", "--json"]);
    assert!(
    ...
    assert_eq!(
        import_payload
            .get("output_schema")
            .and_then(serde_json::Value::as_str),
        Some("isolate://import-response/v1")
    );
}
