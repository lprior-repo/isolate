...
fn test_command_idempotency_under_concurrency() {
    ...
    let workers = 12usize;
    let seed_result = harness.isolate(&[
        "--command-id",
        command_id,
        "add",
        session_name,
    ...
    let state_db = harness.repo_path.join(".isolate").join("state.db");
    ...
    for (index, handle) in handles.into_iter().enumerate() {
        let output = handle
            ...
            .unwrap_or_else(|e| panic!("worker thread {index} failed to spawn isolate: {e}"));
        outputs.push(output);
    }
    ...
    let db_path = harness.repo_path.join(".isolate").join("state.db");
