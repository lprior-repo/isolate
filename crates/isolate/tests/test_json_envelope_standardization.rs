...
fn assert_schema_envelope(json: &serde_json::Value, schema_name: &str) {
    ...
    let expected_schema = format!("isolate://{schema_name}/v1");
    assert_eq!(
...
fn test_done_dry_run_json_has_envelope() -> Result<(), serde_json::Error> {
    ...
    let result = harness.isolate_in_dir(&workspace_path, &["done", "--dry-run", "--json"]);
...
fn test_undo_list_json_has_envelope() -> Result<(), serde_json::Error> {
    ...
    let _ = harness.create_file(".isolate/undo.log", &entry);

    let result = harness.isolate(&["undo", "--list", "--json"]);
...
fn test_revert_dry_run_json_has_envelope() -> Result<(), serde_json::Error> {
    ...
    let result = harness.isolate(&["revert", "revert-test", "--dry-run", "--json"]);
...
fn test_export_json_has_envelope() -> Result<(), serde_json::Error> {
    ...
    let result = harness.isolate(&["export", "--json"]);
