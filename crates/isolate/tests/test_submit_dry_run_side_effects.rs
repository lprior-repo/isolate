...
fn test_submit_dry_run_with_auto_commit_does_not_commit() {
    // GIVEN: An initialized Isolate repository with uncommitted changes
    ...
    let result = harness.isolate(&["submit", "--dry-run", "--auto-commit", "--json"]);
...
fn test_submit_auto_commit_works_when_not_dry_run() {
    ...
    let _ = harness.isolate(&["submit", "--auto-commit", "--json"]);
...
fn test_submit_dry_run_fails_if_dirty_without_auto_commit() {
    ...
    let result = harness.isolate(&["submit", "--dry-run", "--json"]);
