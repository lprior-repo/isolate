...
fn test_submit_succeeds_on_first_submit() {
    // GIVEN: An initialized Isolate repository with a workspace that has changes
    ...
    let result = harness.isolate(&["submit", "--json"]);
...
fn test_submit_duplicate_is_idempotent() {
    // GIVEN: An initialized Isolate repository with a workspace
    let Some(mut harness) = TestHarness::try_new() else {
        return;
    };
    ...
    let first_result = harness.isolate(&["submit", "--json"]);
    ...
    // Run submit again
    let second_result = harness.isolate(&["submit", "--json"]);
