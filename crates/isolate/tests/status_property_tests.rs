...
fn prop_status_output_is_valid_jsonl() {
    ...
    // Create a session
    let result = harness.isolate(&["add", "prop-test", "--no-hooks"]);
    if !result.success {
    ...
    // Query status
    let status_result = harness.isolate(&["status"]);
    ...
    // Cleanup
    let _ = harness.isolate(&["remove", "prop-test", "--merge"]);
}
...
fn prop_status_lines_have_type_discriminator() {
    ...
    let result = harness.isolate(&["add", "prop-type-test", "--no-hooks"]);
    if !result.success {
        println!("SKIP: could not create session");
        return;
    }
    ...
    let _ = harness.isolate(&["remove", "prop-type-test", "--merge"]);
}
...
fn prop_session_lines_have_required_fields() {
    ...
    let result = harness.isolate(&["add", "prop-fields-test", "--no-hooks"]);
    if !result.success {
        println!("SKIP: could not create session");
        return;
    }
    ...
    let _ = harness.isolate(&["remove", "prop-fields-test", "--merge"]);
}
...
fn prop_status_has_valid_summary() {
    ...
    for name in ["prop-summary-1", "prop-summary-2", "prop-summary-3"] {
        let _ = harness.isolate(&["add", name, "--no-hooks"]);
    }
    ...
    // Cleanup
    for name in ["prop-summary-1", "prop-summary-2", "prop-summary-3"] {
        let _ = harness.isolate(&["remove", name, "--merge"]);
    }
}
...
fn prop_session_count_matches_summary() {
    ...
    for name in &session_names {
        let result = harness.isolate(&["add", name, "--no-hooks"]);
        if !result.success {
...
fn test_json_validity_invariant() {
    ...
    let _ = harness.isolate(&["remove", "json-test", "--merge"]);
}
...
fn test_field_completeness_session_name() {
    ...
    let _ = harness.isolate(&["remove", "completeness-test", "--merge"]);
}
...
fn test_summary_line_present() {
    ...
    let _ = harness.isolate(&["remove", "summary-test", "--merge"]);
}
