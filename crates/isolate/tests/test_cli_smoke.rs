...
/// Async helper: Execute isolate command and capture output
async fn run_isolate_async(
    isolate_bin: &PathBuf,
    current_dir: &PathBuf,
    args: &[&str],
...
fn assert_json_output(result: &common::CommandResult, args: &[&str]) {
    ...
    assert!(
        parsed.is_ok(),
        "Expected JSON output for isolate {}\nStdout: {}\nStderr: {}",
        args.join(" "),
        result.stdout,
        result.stderr
    );
...
async fn test_help_for_all_commands() {
    ...
    let isolate_bin = std::sync::Arc::new(harness.isolate_bin.clone());
