...
//! End-to-end integration tests for complete isolate workflows
//!
...
fn test_new_user_onboarding_workflow() {
    // Workflow: User starts fresh, inits isolate, adds first session
    let Some(harness) = TestHarness::try_new() else {
        return;
    };

    // User initializes isolate in their repo
    harness.assert_success(&["init", "--json"]);

    // Verify .isolate directory created
    assert!(harness.isolate_dir().exists());
    ...
    // User checks status
    let result = harness.isolate(&["status", "my-first-feature", "--json"]);
...
fn test_parallel_development_workflow() {
    ...
    let result = harness.isolate(&["status", "feature-auth"]);
    assert!(result.success);
...
fn test_session_switching_workflow() {
    ...
    for session in &["bugfix-123", "feature-456", "experiment-789"] {
        ...
        let result = harness.isolate(&["status", session]);
        assert!(result.success);
    }

    // Focus on different sessions
    let result = harness.isolate(&["focus", "bugfix-123"]);
    assert!(result.success);
    result.assert_output_contains("bugfix-123");

    let result = harness.isolate(&["focus", "feature-456"]);
    assert!(result.success);
...
fn test_json_output_automation_workflow() {
    // Workflow: Automation scripts consuming isolate JSON output
    let Some(harness) = TestHarness::try_new() else {
        return;
    };
...
#[test]
fn test_reinitialize_existing_isolate() {
...
fn test_status_after_multiple_operations() {
    ...
    for _ in 0..3 {
        let result = harness.isolate(&["status", "persistent"]);
        assert!(result.success);
...
fn test_very_long_session_name() {
    ...
    let result = harness.isolate(&["add", &long_name, "--no-open"]);
...
// For now, tests will use the existing isolate() method
