...
fn bdd_remove_contract_flag_does_not_panic() {
    ...
    //   When I run "isolate remove --contract"
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("AI CONTRACT for isolate remove"))
...
fn bdd_remove_contract_flag_works_without_session_name() {
    ...
    //   When I run "isolate remove --contract" (no session name)
...
fn bdd_remove_without_flags_requires_session_name() {
    ...
    //   When I run "isolate remove" (no session name, no --contract)
