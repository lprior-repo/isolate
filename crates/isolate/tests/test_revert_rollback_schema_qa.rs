...
fn rollback_missing_session_returns_not_found_without_duplicate_output() {
    ...
    let result = harness.isolate(&["rollback", "missing-session", "--to", "abc", "--json"]);
...
fn revert_malformed_undo_log_is_reported_as_read_error() {
    ...
    let undo_log = harness.isolate_dir().join("undo.log");
    ...
    let result = harness.isolate(&["revert", "some-session", "--json"]);
...
fn revert_missing_session_returns_semantic_exit_code_without_duplicate_json() {
    ...
    let result = harness.isolate(&["revert", "missing", "--json"]);
    assert!(!result.success);
    assert_eq!(result.exit_code, Some(2));
    let parsed: serde_json::Value = serde_json::from_str(&result.stdout).unwrap_or_default();
    assert_eq!(parsed["success"], false);
