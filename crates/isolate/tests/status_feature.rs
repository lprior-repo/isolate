#![allow(clippy::unnecessary_map_or, clippy::uninlined_format_args)]
...
impl StatusTestContext {
    ...
    pub fn remove_session(&self, name: &str) -> Result<()> {
        let result = self.harness.isolate(&["remove", name, "--merge"]);
        if !result.success {
...
impl Drop for StatusTestContext {
    fn drop(&mut self) {
        ...
        for name in &self.session_names {
            let _ = self.harness.isolate(&["remove", name.as_str(), "--merge"]);
        }
    }
}
...
async fn invariant_json_validity() {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if let Some(schema) = line.get("$schema") {
            ...
            assert!(
                schema_str.starts_with("isolate://"),
                "Invariant violation: $schema should start with 'isolate://'"
            );
        }
