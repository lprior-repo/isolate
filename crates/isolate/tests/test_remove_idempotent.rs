...
//!
//! The contract (rust-contract-isolate-ftds.md) states:
...
fn test_remove_idempotent_succeeds_when_session_doesnt_exist() {
    // GIVEN: An initialized Isolate repository with no session "nonexistent"
    ...
    // WHEN: User runs `isolate remove nonexistent --idempotent`
    let result = harness.isolate(&["remove", "nonexistent", "--idempotent"]);
...
fn test_remove_idempotent_removes_session_when_exists() {
    // GIVEN: An initialized Isolate repository with existing session "old-session"
    let Some(harness) = TestHarness::try_new() else {
        return;
    };
    ...
    // WHEN: User runs `isolate remove old-session --idempotent --force`
    // --force avoids interactive confirmation in test environment.
    let result = harness.isolate(&["remove", "old-session", "--idempotent", "--force"]);
...
fn test_remove_idempotent_with_force_flag_is_redundant() {
    ...
    // WHEN: User runs `isolate remove test --idempotent -f` (both flags)
    let result = harness.isolate(&["remove", "test", "--idempotent", "--force"]);
...
fn test_remove_idempotent_fails_when_not_initialized() {
    ...
    // WHEN: User runs `isolate remove test --idempotent`
    let result = harness.isolate(&["remove", "test", "--idempotent"]);
...
fn test_remove_without_idempotent_fails_on_nonexistent_session() {
    ...
    // WHEN: User runs `isolate remove nonexistent` WITHOUT --idempotent
    let result = harness.isolate(&["remove", "nonexistent", "--force"]);
...
fn test_remove_with_force_succeeds_on_nonexistent_session() {
    ...
    // WHEN: User runs `isolate remove nonexistent --force`
    let result = harness.isolate(&["remove", "nonexistent", "--force"]);
...
fn test_remove_idempotent_json_output_includes_status() {
    ...
    // WHEN: User runs `isolate remove nonexistent --idempotent --json`
    let result = harness.isolate(&["remove", "nonexistent", "--idempotent", "--json"]);
...
fn test_remove_idempotent_never_fails_on_nonexistent_session() {
    ...
    // WHEN: User runs `isolate remove safe-remove --idempotent` multiple times
    let result1 = harness.isolate(&["remove", "safe-remove", "--idempotent"]);
    let result2 = harness.isolate(&["remove", "safe-remove", "--idempotent"]);
    let result3 = harness.isolate(&["remove", "safe-remove", "--idempotent"]);
