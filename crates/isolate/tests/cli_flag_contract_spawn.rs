...
fn bdd_spawn_contract_flag_does_not_panic() {
    ...
    //   When I run "isolate spawn --contract"
    //   Then it should return JSON contract without panic
    //   And exit code should be 0

    let mut cmd = Command::new(env!("CARGO_BIN_EXE_isolate"));
    cmd.arg("spawn").arg("--contract").arg("isolate-test");

    cmd.assert()
        .success()
        .stdout(predicate::str::contains("AI CONTRACT for isolate spawn"))
...
fn bdd_spawn_ai_hints_flag_does_not_panic() {
    ...
    //   When I run "isolate spawn --ai-hints"
    //   Then it should return hints without panic
    //   And exit code should be 0

    let mut cmd = Command::new(env!("CARGO_BIN_EXE_isolate"));
    cmd.arg("spawn").arg("--ai-hints").arg("isolate-test");
...
fn bdd_spawn_with_invalid_bead_still_works() {
    ...
    //   When I run "isolate spawn --contract nonexistent-bead"
...
fn bdd_no_panic_with_any_flag_combination() {
    ...
    let test_cases = vec![
        vec!["spawn", "--contract", "isolate-test"],
        vec!["spawn", "--ai-hints", "isolate-test"],
        vec!["spawn", "--json", "isolate-test"],
        vec!["spawn", "--contract", "--json", "isolate-test"],
        vec!["spawn", "--ai-hints", "--json", "isolate-test"],
        vec!["spawn", "--help"],
    ];
