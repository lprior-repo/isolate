...
//! interactive confirmation from the `isolate remove` command.
//!
//! Test Organization:
...
fn test_hp001_non_interactive_remove_succeeds() {
    // GIVEN an initialized Isolate repository with existing session "feature-auth"
...
fn test_hp002_force_flag_is_no_op() {
    // GIVEN an initialized Isolate repository with existing session "bugfix-123"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
    ...
    // AND behavior is identical to `isolate remove bugfix-123` (force is no-op)
...
fn test_hp004_idempotent_remove_existing_session() {
    // GIVEN an initialized Isolate repository with existing session "test-session"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_hp005_idempotent_remove_missing_session_succeeds() {
    // GIVEN an initialized Isolate repository with NO session "missing-session"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_hp006_dry_run_shows_preview_without_changes() {
    // GIVEN an initialized Isolate repository with existing session "preview-session"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_hp007_json_output_has_correct_schema() {
    // GIVEN an initialized Isolate repository with existing session "json-test"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_hp008_remove_succeeds_when_workspace_already_missing() {
    // GIVEN an initialized Isolate repository with session "orphan-session"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_cv005_session_removed_from_database_postcondition() {
    // GIVEN an initialized Isolate repository with session "verify-removal"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_cv006_workspace_deleted_postcondition() {
    // GIVEN an initialized Isolate repository with session and workspace
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_cv010_force_flag_no_op_invariant() {
    // GIVEN an initialized Isolate repository with session
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
    ...
    // THEN behavior is identical to `isolate remove force-test`
...
fn test_ep001_remove_nonexistent_session_fails() {
    // GIVEN an initialized Isolate repository with NO session "missing"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_ep004_remove_fails_when_not_initialized() {
    ...
    assert!(
        !output.status.success(),
        "Remove should fail when isolate not initialized"
    );
    ...
    assert_eq!(
        output.status.code().unwrap(),
        1,
        "Exit code should be 1 for ValidationError when isolate not initialized"
    );

    // AND error message suggests running `isolate init`
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("isolate init"),
        "Error should suggest running 'isolate init', got: {stderr}"
    );
}

#[test]
fn test_ep010_dry_run_fails_on_nonexistent_session() {
    // GIVEN an initialized Isolate repository with NO session "dry-missing"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
...
fn test_ep025_json_error_output_format() {
    // GIVEN an initialized Isolate repository with NO session "json-error"
    let (_temp_dir, repo_path) = setup_test_repo().expect("Failed to setup test repo");
