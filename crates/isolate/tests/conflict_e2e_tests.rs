...
//! Contract refs:
//! - /home/lewis/src/isolate/contracts/bd-1c4-contract-spec.md
//! - /home/lewis/src/isolate/contracts/bd-1c4-martin-fowler-tests.md
...
fn detect_conflicts_json(harness: &common::TestHarness) -> Option<JsonValue> {
    let result = harness.isolate(&["done", "--detect-conflicts", "--json"]);
    if !result.success {
        return None;
    }
...
fn hp_001_clean_workspace_merge_detection() {
    ...
    // Create workspace
    let result = harness.isolate(&["add", "feature-clean"]);
    if !result.success {
        return;
    }
...
fn hp_008_json_output_format() {
    ...
    let result = harness.isolate(&["add", "feature-json"]);
    if !result.success {
        return;
    }
...
fn hp_010_detection_time_measurement() {
    ...
    let result = harness.isolate(&["add", "feature-timing"]);
    if !result.success {
        return;
    }
...
fn hp_011_quick_conflict_check() {
    ...
    let result = harness.isolate(&["add", "feature-quick"]);
    if !result.success {
        return;
    }
...
fn cv_006_post_det_003_verification() {
    ...
    let result = harness.isolate(&["add", "feature-post3"]);
    if !result.success {
        return;
    }
...
fn cv_017_inv_perf_001_verification() {
    ...
    let result = harness.isolate(&["add", "feature-perf"]);
    if !result.success {
        return;
    }
...
fn e2e_001_full_happy_path_workflow() {
    ...
    // Create workspace
    let result = harness.isolate(&["add", "feature-auth"]);
    if !result.success {
        return;
    }
...
fn e2e_009_recovery_from_interrupted_detection() {
    ...
    let result = harness.isolate(&["add", "feature-recover"]);
    if !result.success {
        return;
    }
