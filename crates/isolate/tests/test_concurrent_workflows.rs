...
// ========================================================================
//
// GIVEN: A fresh isolate repository
...
fn test_parallel_read_operations() {
    ...
    for _ in 0..num_readers {
        let handle = std::thread::spawn(|| {
            ...
            // Read sessions
            let result = local_harness.isolate(&["list", "--json"]);
            result.success
        });
        handles.push(handle);
    }
    ...
    for agent_id in 0..NUM_AGENTS {
        let handle = std::thread::spawn(move || {
            ...
            for session_num in 0..SESSIONS_PER_AGENT {
                ...
                // Create session
                let result = agent_harness.isolate(&["add", &session_name, "--no-open"]);
...
fn test_concurrent_create_delete() {
    ...
    for i in 0..num_operations {
        ...
        let handle = std::thread::spawn(move || {
            ...
            // Delete the session
            let result = local_harness.isolate(&["remove", &session_name, "--force"]);
            result.success
        });
        handles.push(handle);
    }
...
fn test_rapid_operations_stability() {
    ...
    for i in 0..num_operations {
        ...
        // Create
        let create_result = harness.isolate(&["add", &session_name, "--no-open"]);
...
fn test_high_volume_session_management() {
    ...
    for session_name in &created_sessions {
        let result = harness.isolate(&["status", session_name]);
...
fn test_rapid_create_remove_cycles() {
    ...
    for i in 0..num_cycles {
        ...
        let _result = harness.isolate(&["status", &session_name]);
    }
...
fn test_concurrent_status_checks() {
    ...
    for _check_num in 0..num_checks {
        ...
        let handle = std::thread::spawn(move || {
            ...
            for session_name in &sessions_to_check[0..5] {
                let result = local_harness.isolate(&["status", session_name]);
                if !result.success {
                    return false;
                }
            }
