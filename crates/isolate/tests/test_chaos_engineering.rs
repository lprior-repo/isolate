...
//!
//! These tests verify that isolate handles various failure conditions gracefully
...
fn test_chaos_test_harness_integration() {
    ...
    let _result = harness.isolate_with_chaos(&["add", "chaos-test", "--no-open"]);

    let _result = harness.isolate_with_chaos(&["list"]);
...
fn test_recovery_after_chaos() {
    ...
    let normal_result = harness.isolate(&["list"]);
...
fn test_multiple_chaos_cycles() {
    ...
    for i in 0..5 {
        ...
        let status = harness.isolate(&["list"]);
