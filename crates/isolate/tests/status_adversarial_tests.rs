...
fn adversarial_nonexistent_session_exit_code_2() {
    ...
    let result = harness.isolate(&["status", "nonexistent-session-xyz"]);
...
fn adversational_session_with_missing_workspace() {
    ...
    let result = harness.isolate(&["status", "missing-workspace-test"]);
    ...
    let _ = harness.isolate(&["remove", "missing-workspace-test", "--merge"]);
}
...
fn adversarial_session_name_with_special_chars() {
    ...
    for name in &valid_names {
        ...
        if result.success {
            let status_result = harness.isolate(&["status", name]);
            assert!(
...
fn adversarial_output_schema_validation() {
    ...
    let result = harness.isolate(&["status", "schema-test"]);
    ...
    let _ = harness.isolate(&["remove", "schema-test", "--merge"]);
}
