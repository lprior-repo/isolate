...
// Test lock TTL expiry with short TTL (1s)
#[test]
fn test_lock_ttl() {
    let Some(harness) = TestHarness::try_new() else {
        return;
    ...
    // Lock with 1s TTL
    harness.assert_success(&["lock", "lock-test", "--ttl", "1", "--agent-id", "agent1"]);

    // Immediately try to lock as another agent - should fail
    let result = harness.isolate(&["lock", "lock-test", "--agent-id", "agent2"]);
    assert!(
        !result.success,
        "Should not be able to lock while another agent holds it"
    );

    // Wait for TTL to expire (1.1s gives margin over 1s TTL)
...
fn test_lock_agent_id_enforcement() {
    ...
    let result = harness.isolate(&["unlock", "agent-lock", "--agent-id", "agent2"]);
    assert!(
        !result.success,
...
fn test_done_detect_conflicts() {
    ...
    let result = harness.isolate(&["done", "--workspace", "conflict-ws", "--detect-conflicts"]);
