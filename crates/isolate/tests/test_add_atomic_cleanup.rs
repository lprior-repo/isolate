...
fn test_add_workspace_creation_failure_rolls_back_file_workspace_path() {
    ...
    let result = harness.isolate(&["add", session_name, "--no-open", "--no-hooks"]);
    assert!(
        !result.success,
    ...
    let list_result = harness.isolate(&["list", "--all", "--json"]);
    assert!(
        list_result.success,
        "list should succeed: {}",
        list_result.stderr
    );
    let parsed_result = common::parse_jsonl_output(&list_result.stdout);
    assert!(
        parsed_result.is_ok(),
        "list output should be valid jsonl: {}",
        list_result.stdout
    );
    ...
    let retry_result = harness.isolate(&["add", session_name, "--no-open", "--no-hooks"]);
    assert!(
    ...
    let retry_list_result = harness.isolate(&["list", "--all", "--json"]);
    assert!(
        retry_list_result.success,
        "list should succeed after retry: {}",
        retry_list_result.stderr
    );
    let retry_parsed_result = common::parse_jsonl_output(&retry_list_result.stdout);
    assert!(
        retry_parsed_result.is_ok(),
        "list output should be valid jsonl after retry: {}",
        retry_list_result.stdout
    );
    let retry_parsed = retry_parsed_result.unwrap();
    let session_status = retry_parsed.iter().find_map(|line| {
        line.get("session")
            .filter(|s| s.get("name").and_then(|n| n.as_str()) == Some(session_name))
...
fn test_add_hook_failure_marks_failed_and_rolls_back_workspace() {
    ...
    let result = harness.isolate_with_env(
        ...
        &[("Isolate_TEST_FAIL_POST_CREATE_HOOK", "1")],
    );
    assert!(
        !result.success,
    ...
    let parsed = parsed_result.unwrap();
    let session_status = parsed.iter().find_map(|line| {
        line.get("session")
            .filter(|s| s.get("name").and_then(|n| n.as_str()) == Some(session_name))
            .and_then(|s| s.get("status"))
            .and_then(|s| s.as_str())
...
fn test_add_hook_failure_recoverable_via_remove_then_retry() {
    ...
    let failed_add = harness.isolate_with_env(
        &["add", session_name, "--no-open"],
        &[("Isolate_TEST_FAIL_POST_CREATE_HOOK", "1")],
    );
    assert!(
