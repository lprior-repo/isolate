...
fn bdd_help_supports_nested_subcommand_paths() {
    // Given a compiled isolate binary
    ...
    let result = harness.isolate(&["help", "pane", "focus"]);
...
fn bdd_help_unknown_nested_path_reports_full_path() {
    ...
    let result = harness.isolate(&["help", "pane", "does-not-exist"]);
...
fn bdd_import_rejects_conflicting_force_and_skip_existing_flags() {
    ...
    let result = harness.isolate(&["import", "--force", "--skip-existing", "input.json"]);
...
fn bdd_import_rejects_pre_epoch_timestamps() {
  ...
  "sessions": [
    ...
    let result = harness.isolate(&["import", &import_path]);
...
fn bdd_pane_focus_rejects_pane_and_direction_together() {
    ...
    let result = harness.isolate(&["pane", "focus", "test-session", "3", "--direction", "left"]);
