...
fn test_sigterm_during_session_creation() {
    ...
    let add_result = harness.isolate(&["add", session_name, "--no-open"]);
    ...
    let status_result = harness.isolate(&["status", session_name]);
...
fn test_rapid_add_remove_interruption_cycle() {
    ...
    let completed_cycles = (0..cycle_count)
        .map(|i| {
            ...
            let remove_result = harness.isolate(&["remove", &session_name, "--force"]);
...
fn test_shutdown_during_active_operations() {
    ...
    let create_results: Vec<_> = session_names
        .iter()
        .map(|name| harness.isolate(&["add", name, "--no-open"]))
        .collect();
    ...
    let _remove_results: Vec<_> = session_names
        .iter()
        .map(|name| harness.isolate(&["remove", name, "--force"]))
        .collect();
}
...
fn test_concurrent_list_operations_with_interruption() {
    ...
    let list_results: Vec<_> = (0..5).map(|_| harness.isolate(&["list"])).collect();
...
fn test_database_consistency_after_interruption() {
    ...
    let _ = harness.isolate(&["remove", "test1", "--force"]);
    ...
    // Verify final state
    let final_result = harness.isolate(&["list"]);
...
fn test_no_orphaned_workspaces_after_interrupted_add() {
    ...
    let created_sessions: Vec<String> = (0..3)
        ...
        .filter_map(|session_name| {
            let result = harness.isolate(&["add", &session_name, "--no-open"]);
            if result.success {
                Some(session_name)
            } else {
                None
    ...
    let _cleanup_results: Vec<_> = created_sessions
        .iter()
        .map(|session| harness.isolate(&["remove", session, "--force"]))
        .collect();
}
...
fn test_cleanup_after_failed_operations() {
    ...
    let _ = harness.isolate(&["add", "valid-name", "--no-open"]);

    // Try invalid operations
    let _ = harness.isolate(&["add", "", "--no-open"]);
    let _ = harness.isolate(&["add", "invalid@name", "--no-open"]);
    let _ = harness.isolate(&["remove", "nonexistent", "--force"]);
...
fn test_multiple_rapid_signals() {
    ...
    let operation_results: Vec<_> = (0..iteration_count)
        .flat_map(|_| {
            [
                harness.isolate(&["status", "signal-test"]),
                harness.isolate(&["list"]),
            ]
        })
        .collect();
    ...
    let result = harness.isolate(&["status", "signal-test"]);
...
fn test_interruption_during_status_check() {
    ...
    let status_results: Vec<_> = (0..check_count)
        .map(|_| harness.isolate(&["status", "status-interrupt"]))
        .collect();
    ...
    let result = harness.isolate(&["status", "status-interrupt"]);
    assert!(
        result.success,
...
fn test_recovery_from_partial_state() {
    ...
    let _status_result = harness.isolate(&["status", "recovery-1"]);
    ...
    // Cleanup
    let _ = harness.isolate(&["remove", "recovery-0", "--force"]);
    let _ = harness.isolate(&["remove", "recovery-2", "--force"]);
}
...
fn test_state_consistency_after_timeout() {
    ...
    let status_results: Vec<_> = (0..check_count)
        .map(|_| harness.isolate(&["status", "timeout-test"]))
        .collect();
    ...
    let final_result = harness.isolate(&["status", "timeout-test"]);
    assert!(
