...
fn run_isolate_with_env(
    ...
    let mut command = Command::new(&harness.isolate_bin);
    command
        .args(args)
...
fn given_clean_age_threshold_when_non_numeric_then_cli_rejects_value() {
    ...
    let result = harness.isolate(&["clean", "--age-threshold", "abc"]);
...
fn given_boolean_config_when_setting_string_value_then_command_fails_and_config_stays_readable() {
    ...
    let invalid_set = harness.isolate(&["config", "watch.enabled", "notabool"]);
    assert!(
        !invalid_set.success,
    ...
    let lookup = harness.isolate(&["config", "watch.enabled"]);
    assert!(
        lookup.success,
...
fn given_global_and_project_values_when_reading_with_global_flag_then_global_value_is_returned() {
    ...
    let global_set = run_isolate_with_env(
        &harness,
    ...
    let global_read = run_isolate_with_env(&harness, &["config", "--global", "main_branch"], &envs);
    assert!(
        global_read.success,
