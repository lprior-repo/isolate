...
fn test_workspace_exists_without_db_entry() {
    ...
    // - `isolate list` shows nothing (no DB entry)
    // - `isolate add same-name` fails because workspace already exists
    ...
    let result = harness.isolate(&["add", "zombie-session", "--no-open"]);

    // EXPECTED: Either succeeds (recovering the orphan) OR fails with clear error
    if result.success {
        let list_after = harness.isolate(&["list", "--json"]);
        assert!(
            list_after.success,
...
fn test_db_entry_exists_without_workspace() {
    ...
    // CURRENT CHAMPION BEHAVIOR:
    // - DB entry exists
    // - `isolate list` shows the session
    // - `isolate status` fails because workspace is missing
    //
    // EXPECTED BEHAVIOR:
    ...
    // Delete the workspace behind isolate's back
    ...
    let query_result = harness.isolate(&["query", "session-exists", "ghost-session", "--json"]);
...
fn test_clean_command_detects_orphans() {
    ...
    //
    // CURRENT CHAMPION BEHAVIOR:
    // - `isolate clean` only removes sessions from DB that have no workspace
    // - Does NOT detect workspaces that have no DB entry
    //
    // EXPECTED BEHAVIOR:
    // - `isolate clean --detect-orphans` finds workspaces without DB entries
    ...
    // Run clean
    let clean_result = harness.isolate(&["clean", "--dry-run", "--json"]);
...
fn test_recover_orphaned_session() {
    ...
    //
    // EXPECTED BEHAVIOR:
    // - `isolate add --recover orphan-session` detects workspace and creates DB entry
    let Some(harness) = TestHarness::try_new() else {
        // Test framework will handle skipping - no output needed
        return;
    };
    ...
    let result = harness.isolate(&["add", "recoverable-session", "--no-open"]);
...
fn test_add_is_not_idempotent() {
    ...
    let result = harness.isolate(&["add", "idemp-test", "--no-open"]);
...
fn test_remove_is_somewhat_idempotent() {
    ...
    let _result = harness.isolate(&["remove", "never-existed", "-f"]);
...
fn test_lock_cleanup_on_query() {
    ...
    // This would require a new command like `isolate lock acquire`
    //
...
// ============================================================================
//
// These tests define the fitness signal for isolate:
//
