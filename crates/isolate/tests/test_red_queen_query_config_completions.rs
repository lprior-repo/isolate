...
fn bdd_query_session_count_accepts_status_filter_flag_like_value() {
    ...
    let result = harness.isolate(&["query", "session-count", "--status=active"]);
    assert!(
        result.success,
...
fn bdd_query_session_count_rejects_unknown_filter_syntax() {
    ...
    let result = harness.isolate(&["query", "session-count", "bogus-filter"]);
    assert!(
        !result.success,
...
fn bdd_query_session_count_rejects_unknown_status_value() {
    ...
    let result = harness.isolate(&["query", "session-count", "--status=nonesuch"]);
    assert!(
        !result.success,
...
fn bdd_query_location_rejects_unexpected_argument() {
    ...
    let result = harness.isolate(&["query", "location", "unexpected-arg"]);
    assert!(
        !result.success,
...
fn bdd_query_pending_merges_rejects_unexpected_argument() {
    ...
    let result = harness.isolate(&["query", "pending-merges", "unexpected-arg"]);
    assert!(
        !result.success,
...
fn bdd_query_can_run_unknown_command_reports_unknown_command_blocker() {
    ...
    let result = harness.isolate(&["query", "can-run", "totally-unknown-cmd", "--json"]);
    assert!(
        result.success,
...
fn bdd_query_session_count_json_mode_returns_schema_envelope() {
    ...
    assert_eq!(
        ...
        Some("isolate://query-session-count/v1"),
...
fn bdd_query_suggest_name_json_mode_returns_error_envelope_on_validation_failure() {
    ...
    let result = harness.isolate(&["query", "suggest-name", "feat", "--json"]);
    assert!(
        !result.success,
    ...
    assert_eq!(
        parsed.get("$schema").and_then(serde_json::Value::as_str),
        Some("isolate://error-response/v1"),
...
fn bdd_query_can_spawn_with_bead_fails_when_br_is_unavailable() {
    ...
    let result = harness.isolate_with_env(
        &["query", "can-spawn", "isolate-no-such-bead", "--json"],
        &[("PATH", "/usr/bin")],
    );
    assert!(
        result.success,
...
fn bdd_query_can_spawn_reports_not_in_jj_repository_explicitly() {
    ...
    let isolate_bin = std::path::PathBuf::from(env!("CARGO_BIN_EXE_isolate"));
    let output = std::process::Command::new(&isolate_bin)
        .args(["query", "can-spawn", "--json"])
        .current_dir(temp_dir.path())
        .env("NO_COLOR", "1")
        .output()
        .expect("run isolate query can-spawn in non-repo dir");
...
fn bdd_config_global_key_read_uses_global_scope() {
    ...
    let isolate_global_dir = xdg_config_home.join("isolate");
    std::fs::create_dir_all(&isolate_global_dir).expect("create global config dir");
    std::fs::write(
        isolate_global_dir.join("config.toml"),
        "workspace_dir = \"../GLOBAL\"\n",
    )
    .expect("write global config");

    std::fs::create_dir_all(harness.isolate_dir()).expect("create project .isolate dir");
    std::fs::write(
        harness.isolate_dir().join("config.toml"),
...
fn bdd_config_empty_array_round_trips_without_phantom_empty_item() {
    ...
    let set_result = harness.isolate(&["config", "watch.paths", "[]", "--json"]);
    assert!(
        set_result.success,
    ...
    let get_result = harness.isolate(&["config", "watch.paths", "--json"]);
    assert!(
        get_result.success,
...
fn bdd_config_rejects_non_string_array_elements() {
    ...
    let result = harness.isolate(&["config", "watch.paths", "[1, 2]", "--json"]);
    assert!(
        !result.success,
...
fn bdd_completions_accepts_pwsh_alias() {
    ...
    let result = harness.isolate(&["completions", "pwsh"]);
    assert!(
        result.success,
...
fn bdd_completions_accepts_case_insensitive_shell_name() {
    ...
    let result = harness.isolate(&["completions", "BASH"]);
    assert!(
        result.success,
        "Expected uppercase shell to succeed\nstdout: {}\nstderr: {}",
        result.stdout, result.stderr
    );
    result.assert_output_contains("_isolate()");
}
