...
fn test_spawn_with_corrupted_bead_database() {
    ...
    // Initialize isolate in the test repository
...
{invalid json missing closing brace"#;
    ...
    let result = harness.isolate(&["spawn", "corrupt-me", "--agent-command", "echo"]);
...
fn test_spawn_with_malformed_json_in_database() {
    ...
    // Attempt spawn
    let result = harness.isolate(&["spawn", "any-bead", "--agent-command", "echo"]);
...
fn test_spawn_validates_bead_status_before_workspace_creation() {
    ...
    let result = harness.isolate(&["spawn", "blocked-bead", "--agent-command", "echo"]);
...
"#;
    ...
    let result = harness.isolate(&["spawn", "valid-bead", "--agent-command", "echo"]);
...
{"id":"dup-bead","title":"Second Instance","status":"open","priority":"2","issue_type":"task","created_at":"2026-01-30T00:01:00Z","updated_at":"2026-01-30T00:01:00Z","source_repo":"."}"#;
    ...
    let result = harness.isolate(&["spawn", "dup-bead", "--agent-command", "echo"]);
...
{invalid json"#;
    ...
    let result = harness.isolate(&["spawn", "corrupt-entry", "--agent-command", "echo"]);
