// BDD Test for --contract and --ai-hints flags on focus command
//
...
#[test]
fn bdd_focus_contract_flag_does_not_panic() {
    // Scenario: AI queries focus command contract
    //   When I run "isolate focus --contract"
    //   Then it should return JSON contract without panic
    //   And exit code should be 0

    let mut cmd = Command::new(env!("CARGO_BIN_EXE_isolate"));
    cmd.arg("focus").arg("--contract");

    cmd.assert()
        .success()
        .stdout(predicate::str::contains("AI CONTRACT for isolate focus"))
...
#[test]
fn bdd_focus_ai_hints_flag_does_not_panic() {
    // Scenario: AI queries focus command execution hints
    //   When I run "isolate focus --ai-hints"
    //   Then it should return hints without panic
    //   And exit code should be 0

    let mut cmd = Command::new(env!("CARGO_BIN_EXE_isolate"));
    cmd.arg("focus").arg("--ai-hints");
...
#[test]
fn bdd_focus_without_flags_runs_normally() {
    // Scenario: Normal focus operation still works
    //   When I run "isolate focus --help"
    //   Then it should show help including new flags
    //   And exit code should be 0

    let mut cmd = Command::new(env!("CARGO_BIN_EXE_isolate"));
    cmd.arg("focus").arg("--help");
...
#[test]
fn bdd_focus_no_panic_with_any_flag_combination() {
    ...
    let test_cases = vec![
        ...
        vec!["focus", "--ai-hints"],
        vec!["focus", "--json"],
        vec!["focus", "--contract", "--json"],
        vec!["focus", "--ai-hints", "--json"],
        vec!["focus", "--help"],
    ];
