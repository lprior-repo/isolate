...
impl StatusAtddContext {
    ...
    pub fn init(&self) -> Result<()> {
        let result = self.harness.isolate(&["init"]);
        if !result.success {
            anyhow::bail!("Failed to initialize Isolate: {}", result.stderr);
        }
        Ok(())
    }
    ...
    pub fn create_session(&mut self, name: &str) -> Result<()> {
        let result = self.harness.isolate(&["add", name, "--no-hooks"]);
        if result.success {
...
impl Drop for StatusAtddContext {
    fn drop(&mut self) {
        ...
        for name in &self.session_names {
            let _ = self.harness.isolate(&["remove", name, "--merge"]);
        }
    }
}
...
/// WHEN: I run "isolate status my-feature"
...
/// WHEN: I run "isolate status nonexistent"
...
/// WHEN: I run "isolate status --json"
...
/// WHEN: I run "isolate status readonly" twice
...
/// WHEN: I run "isolate status workspace-test"
...
/// WHEN: I run "isolate status status-test"
...
fn invariant_json_always_valid() {
    ...
    for cmd in commands {
        let result = ctx.harness.isolate(&cmd);
...
/// WHEN: I run "isolate status feature-123"
