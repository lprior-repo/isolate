...
/// unit tests in `isolate_core::output::writer`.
...
mod focus_command {
    ...
    fn setup_focused_session(harness: &TestHarness) -> Result<String> {
        ...
        // Initialize isolate
        harness.isolate(&["init"]).assert_success();
    ...
    fn test_focus_missing_session_emits_issue() -> Result<()> {
        ...
        let result = harness.isolate(&["focus", "nonexistent-session"]);
    ...
    fn test_focus_no_name_emits_validation_issue() -> Result<()> {
        ...
        let result = harness.isolate(&["focus"]);
...
mod remove_command {
    ...
    fn test_remove_success_all_lines_valid_jsonl() -> Result<()> {
        ...
        // Remove the session
        let result = harness.isolate(&["remove", session_name, "--force"]);
    ...
    fn test_remove_missing_session_fails() -> Result<()> {
        ...
        let result = harness.isolate(&["remove", "nonexistent"]);
...
mod sync_command {
    ...
    fn test_sync_success_all_lines_valid_jsonl() -> Result<()> {
        ...
        // Sync the session
        let result = harness.isolate(&["sync", session_name]);
    ...
    fn test_sync_all_counts_consistent() -> Result<()> {
        ...
        // Sync all
        let _result = harness.isolate(&["sync", "--all"]);
...
mod add_command {
    ...
    fn test_add_invalid_name_emits_validation_issue() -> Result<()> {
        ...
        let result = harness.isolate(&["add", "123-invalid", "--no-hooks"]);
