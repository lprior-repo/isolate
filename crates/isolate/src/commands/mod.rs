...
pub async fn check_jj_installed() -> Result<()> {
    isolate_core::jj::check_jj_installed().await.map_err(|_| {
        anyhow::anyhow!(
...
pub async fn check_in_jj_repo() -> Result<PathBuf> {
    isolate_core::jj::check_in_jj_repo().await.map_err(|_| {
        anyhow::anyhow!(
            "Not in a JJ repository.\n\n\
            Run 'isolate init' to initialize JJ and Isolate in this directory."
        )
    })
}
...
async fn isolate_project_root() -> Result<PathBuf> {
...
/// Get the Isolate data directory for the current repository
///
/// # Errors
///
/// Returns an error if prerequisites are not met (JJ not installed or not in a JJ repo)
pub async fn isolate_data_dir() -> Result<PathBuf> {
    // Check prerequisites first
    let root = isolate_project_root().await?;
    Ok(root.join(".isolate"))
}
...
pub async fn get_db_path() -> Result<PathBuf> {
    let repo_root = isolate_project_root().await?;
...
pub async fn get_session_db() -> Result<SessionDb> {
    ...
    if std::env::var("Isolate_STATE_DB").is_err() {
        let data_dir = isolate_data_dir().await?;
        anyhow::ensure!(
            tokio::fs::try_exists(&data_dir).await.unwrap_or(false),
            "Isolate not initialized. Run 'isolate init' first."
        );
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_isolate_data_dir_checks_prerequisites() {
        // isolate_data_dir should call check_prerequisites
        let result = isolate_data_dir().await;
        ...
        if let Err(e) = result {
            ...
            assert!(
                msg.contains("JJ is not installed")
                    || msg.contains("Not in a JJ repository")
                    || msg.contains("Failed to execute jj"),
                "isolate_data_dir should fail with prerequisite errors when not met"
            );
        } else {
            ...
            assert!(
                path.is_some(),
                "isolate_data_dir should return a path when prerequisites are met"
            );
            if let Some(p) = path {
                assert!(
                    p.to_string_lossy().ends_with(".isolate"),
                    "Path should end with .isolate"
                );
            }
        }
    }
    ...
    async fn test_get_session_db_requires_init() {
        // get_session_db should fail if isolate is not initialized
        // Even if we're in a JJ repo, if .isolate doesn't exist, it should fail
        ...
        if let Err(e) = result {
            ...
            assert!(
                msg.contains("JJ is not installed")
                    || msg.contains("Not in a JJ repository")
                    || msg.contains("Isolate not initialized")
                    || msg.contains("Failed to execute jj")
    ...
    async fn test_prerequisite_error_messages_are_actionable() {
        ...
        if let Err(e) = repo_err {
            let msg = e.to_string();
            // If we get the "not in repo" error, it should mention isolate init
            if msg.contains("Not in a JJ repository") {
                assert!(
                    msg.contains("isolate init"),
                    "Repository error should mention 'isolate init'"
                );
            }
        }
    }
