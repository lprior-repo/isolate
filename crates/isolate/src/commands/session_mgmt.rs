...
pub async fn run_clone(options: &CloneOptions) -> Result<()> {
    ...
    // Map isolate_core::Error to anyhow::Error while preserving the original error
    ...
    let new_workspace_path = if source_path.is_empty() {
        ...
        if let Some(new_path) = &new_path {
            // Use isolate_core to create workspace (async)
            isolate_core::jj::workspace_create(&options.target, new_path)
                .await
    ...
    fn test_clone_rejects_non_ascii_target_name() {
        ...
        for name in invalid_names {
            ...
            if let Err(e) = result {
                ...
                assert!(
                    matches!(e, isolate_core::Error::ValidationError { .. }),
