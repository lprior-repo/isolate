...
/// Test: `isolate sync` with no arguments should sync current workspace
///
/// When run from within a isolate workspace, should detect the workspace
...
/// Test: `isolate sync <name>` should sync the named session
///
...
/// Test: `isolate sync --all` should sync all active sessions
///
...
/// Test: `isolate sync` from main repo (not in workspace) should sync all
///
...
async fn test_sync_nonexistent_session_returns_error() -> anyhow::Result<()> {
    ...
    // Err(anyhow::Error::new(isolate_core::Error::NotFound(...)))
