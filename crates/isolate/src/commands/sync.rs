...
//! |---------------------|----------------|----------------------------|
//! | `isolate sync <name>`   | Any            | Sync ONLY the named session |
//! | `isolate sync --all`    | Any            | Sync ALL active sessions    |
//! | `isolate sync`          | In workspace   | Sync ONLY current workspace |
//! | `isolate sync`          | In main repo   | Sync ALL sessions (prompt)  |
//!
//! ## Safety Guarantees
//!
//! 1. **Named sync is isolated**: `isolate sync <name>` only affects that session
//! 2. **Workspace sync is local**: `isolate sync` from workspace syncs only that workspace
    ...
    if !output.status.success() {
        return Err(anyhow::anyhow!(
            "Not in a JJ repository. \
             'isolate sync' must be run from within a JJ repository."
        ));
    }
...
///
/// This implements the safe default for `isolate sync` with no arguments:
///
...
async fn sync_session_with_options(name: &str, options: SyncOptions) -> Result<()> {
    ...
    if options.format.is_json() {
        ...
        println!("  isolate done          # Merge to main + cleanup");
    }
...
async fn sync_session_internal(
    ...
    // This serializes syncs across all isolate processes for this repo
    let data_dir = crate::commands::isolate_data_dir().await?;
...
mod tests {
    ...
    fn test_sync_jsonl_action_output_on_success() -> anyhow::Result<()> {
        use isolate_core::output::{Action, ActionStatus, ActionTarget, ActionVerb, OutputLine};
    ...
    fn test_sync_jsonl_result_output_on_success() -> anyhow::Result<()> {
        use isolate_core::output::{Message, OutputLine, ResultKind, ResultOutput};
    ...
    fn test_sync_jsonl_summary_output() -> anyhow::Result<()> {
        use isolate_core::output::{Message, OutputLine, Summary, SummaryType};
