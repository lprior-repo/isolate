...
const AI_INSTRUCTIONS: &str = r"# AI Agent - You Are in a Isolate Workspace

## STOP - Do NOT Clone Elsewhere

You were invoked via `isolate spawn` and are **already in the correct isolated workspace**.

- **Work here** - Do NOT clone this repository to another location
- **Current workspace**: `.isolate/workspaces/<bead-id>/`
- **Your task**: Defined by `$Isolate_BEAD_ID`

## Environment Variables

- `Isolate_BEAD_ID` - The bead/issue you're working on
- `Isolate_WORKSPACE` - Path to this isolated workspace

## When Done

Just exit cleanly with success (exit code 0). Isolate will automatically:
...
async fn handle_early_exit(
    ...
    let workspaces_dir = Path::new(root).join(".isolate/workspaces");
...
fn output_result(result: &SpawnOutput, format: isolate_core::OutputFormat) -> Result<()> {
    if format.is_json() {
        ...
        if matches!(result.status, SpawnStatus::Running) {
            ...
            println!("  isolate sync          # Preview changes / sync with main");
