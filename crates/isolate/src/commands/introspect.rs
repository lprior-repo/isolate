//! Introspect command - discover isolate capabilities
//!
...
///
/// ```
/// # use isolate::commands::introspect::capitalize_category;
...
fn create_add_error_conditions() -> Vec<ErrorCondition> {
    vec![
        create_error_condition(
            ...
            "Choose a different session name or remove the existing session with 'isolate remove'",
        ),
        create_error_condition(
...
fn create_remove_error_conditions() -> Vec<ErrorCondition> {
    vec![create_error_condition(
        ...
        "List active sessions with 'isolate list' to verify the session name",
    )]
}
...
fn get_init_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        examples: vec![CommandExample {
            command: "isolate init".to_string(),
...
fn get_focus_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        flags: vec![],
        examples: vec![CommandExample {
            command: "isolate focus my-session".to_string(),
        ...
        error_conditions: vec![ErrorCondition {
            ...
            resolution: "Check session name with 'isolate list'".to_string(),
        }],
    }
}
...
fn get_status_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        examples: vec![
            CommandExample {
                command: "isolate status".to_string(),
                description: "Show status of all sessions".to_string(),
            },
            CommandExample {
                command: "isolate status my-session".to_string(),
...
fn get_sync_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        flags: vec![],
        examples: vec![CommandExample {
            command: "isolate sync my-session".to_string(),
...
fn get_diff_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        examples: vec![
            CommandExample {
                command: "isolate diff my-session".to_string(),
                description: "Show full diff".to_string(),
            },
            CommandExample {
                command: "isolate diff my-session --stat".to_string(),
...
fn get_introspect_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        examples: vec![
            CommandExample {
                command: "isolate introspect".to_string(),
                description: "Show all capabilities".to_string(),
            },
            CommandExample {
                command: "isolate introspect add --json".to_string(),
...
fn get_doctor_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        examples: vec![
            CommandExample {
                command: "isolate doctor".to_string(),
                description: "Check system health".to_string(),
            },
            CommandExample {
                command: "isolate doctor --fix".to_string(),
...
fn get_query_introspection() -> CommandIntrospection {
    CommandIntrospection {
        ...
        examples: vec![
            CommandExample {
                command: "isolate query session-exists my-session".to_string(),
                description: "Check if session exists".to_string(),
            },
            CommandExample {
                command: "isolate query can-run add".to_string(),
                description: "Check if add command can run".to_string(),
            },
            CommandExample {
                command: "isolate query suggest-name feature-{n}".to_string(),
    ...
    let workflows = vec![
        WorkflowPattern {
            ...
            steps: vec![
                WorkflowStep {
                    step: 1,
                    command: "isolate work my-task --idempotent".to_string(),
                    description: "Create workspace (idempotent for retries)".to_string(),
                },
                WorkflowStep {
                    step: 2,
                    command: "cd $(isolate context --field location.path)".to_string(),
                ...
                WorkflowStep {
                    step: 4,
                    command: "isolate done".to_string(),
        ...
        WorkflowPattern {
            ...
            steps: vec![
                WorkflowStep {
                    step: 1,
                    command: "isolate agent register".to_string(),
                    description: "Register as an agent".to_string(),
                },
                WorkflowStep {
                    step: 2,
                    command: "isolate work my-task --bead isolate-abc12".to_string(),
                    description: "Create workspace for bead".to_string(),
                },
                WorkflowStep {
                    step: 3,
                    command: "isolate agent heartbeat --command \"implementing\"".to_string(),
                ...
                WorkflowStep {
                    step: 5,
                    command: "isolate agent unregister".to_string(),
        ...
        WorkflowPattern {
            ...
            steps: vec![
                WorkflowStep {
                    step: 1,
                    command: "isolate whereami".to_string(),
                    description: "Check location: main or workspace".to_string(),
                },
                WorkflowStep {
                    step: 2,
                    command: "isolate whoami".to_string(),
                    description: "Check agent identity".to_string(),
                },
                WorkflowStep {
                    step: 3,
                    command: "isolate query can-spawn".to_string(),
        ...
        WorkflowPattern {
            ...
            steps: vec![
                WorkflowStep {
                    step: 1,
                    command: "isolate abort --dry-run".to_string(),
                    description: "Preview what will be aborted".to_string(),
                },
                WorkflowStep {
                    step: 2,
                    command: "isolate abort".to_string(),
        ...
        WorkflowPattern {
            ...
            steps: vec![WorkflowStep {
                step: 1,
                command: "isolate sync --all".to_string(),
...
pub struct AiQuickRef {
    /// Whether isolate is ready to use
    pub ready: bool,
...
pub async fn run_ai() -> Result<()> {
    ...
    if !system_state.initialized {
        blockers.push("Isolate not initialized (run 'isolate init')".to_string());
    }

    // Get agent ID from environment
    let agent_id = std::env::var("Isolate_AGENT_ID").ok();
    ...
    if !system_state.initialized && system_state.jj_repo {
        recommendations.push(AiRecommendation {
            ...
            command: "isolate init".to_string(),
            reason: "Isolate is not initialized in this repository".to_string(),
    ...
    if ready && system_state.active_sessions == 0 {
        recommendations.push(AiRecommendation {
            ...
            command: "isolate work <task-name>".to_string(),
    ...
    if ready && location.starts_with("workspace:") {
        recommendations.push(AiRecommendation {
            ...
            command: "isolate context".to_string(),
    ...
    let workflows = vec![
        ...
        AiWorkflowSummary {
            ...
            commands: vec![
                ...
                "isolate work <name> --bead <id>".to_string(),
                "isolate agent heartbeat".to_string(),
                "isolate done".to_string(),
                "isolate agent unregister".to_string(),
            ],
        },
        AiWorkflowSummary {
...
pub fn run_session_states(format: OutputFormat) -> Result<()> {
    ...
    let transitions = vec![
        StateTransition {
            ...
            trigger: "isolate add / isolate work".to_string(),
        },
        StateTransition {
        ...
        StateTransition {
            ...
            trigger: "isolate sync".to_string(),
        },
        StateTransition {
        ...
        StateTransition {
            ...
            trigger: "isolate done".to_string(),
        },
        StateTransition {
        ...
        StateTransition {
            from: "active".to_string(),
            to: "failed".to_string(),
            trigger: "isolate abort".to_string(),
        },
    ];
...
mod tests {
    ...
    fn test_introspect_env_vars_output_structure() {
        ...
        for var in expected_vars {
            assert!(!var.is_empty());
            assert!(var.starts_with("Isolate_"));
        }
    }
    ...
    fn test_introspect_env_vars_contains_core_vars() {
        ...
        let core_vars = [
            ...
            ("Isolate_WORKSPACE", "Workspace path"),
            ("Isolate_ACTIVE", "Whether isolate is active"),
    ...
    fn test_introspect_workflows_output_structure() {
        ...
        let workflow = json!({
            ...
            "steps": [
                {"step": 1, "command": "isolate work <name>", "description": "Start work"},
                {"step": 2, "command": "# do work", "description": "Implementation"},
                {"step": 3, "command": "isolate done", "description": "Complete work"}
            ]
        });
    ...
    fn test_introspect_env_vars_has_examples() {
        ...
        let var_with_examples = [
            ("Isolate_SESSION", "echo $Isolate_SESSION"),
            ("Isolate_WORKSPACE", "cd $Isolate_WORKSPACE"),
            ("Isolate_AGENT_ID", "export Isolate_AGENT_ID=my-agent"),
        ];
