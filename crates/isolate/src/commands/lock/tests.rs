...
use isolate_core::coordination::locks::LockManager;
...
async fn test_pool() -> Result<sqlx::SqlitePool, isolate_core::Error> {
    use sqlx::sqlite::SqlitePoolOptions;
    SqlitePoolOptions::new()
        .connect("sqlite::memory:")
        .await
        .map_err(|e| isolate_core::Error::DatabaseError(e.to_string()))
}

/// Helper to setup lock manager for tests
async fn setup_lock_manager() -> Result<LockManager, isolate_core::Error> {
...
async fn setup_lock_manager_with_ttl(ttl_secs: i64) -> Result<LockManager, isolate_core::Error> {
    let pool = test_pool().await?;
...
// EARS 5: WHEN agent_id not provided, use Isolate_AGENT_ID env var
#[tokio::test]
async fn test_lock_uses_env_agent_id() -> anyhow::Result<()> {
    let mgr = setup_lock_manager().await?;

    std::env::set_var("Isolate_AGENT_ID", "env-agent");
