...
async fn run_cleanup_iteration(config: &PeriodicCleanupConfig) -> Result<PeriodicCleanupOutput> {
    ...
    let db_path = std::path::Path::new(".isolate/beads.db");
    let recovery_config = isolate_core::config::RecoveryConfig {
        policy: if config.dry_run {
            isolate_core::RecoveryPolicy::Silent
        } else {
            isolate_core::RecoveryPolicy::Warn
        },
        log_recovered: true.into(),
        auto_recover_corrupted_wal: true.into(),
        delete_corrupted_database: false.into(),
    };
    let recovered_count = isolate_core::recover_incomplete_sessions(db_path, &recovery_config)
        .await
