...
async fn get_db_pool() -> Result<SqlitePool> {
    ...
    if !db_path.exists() {
        anyhow::bail!("Isolate not initialized. Run 'isolate init' first.");
    }
...
pub async fn run_register(args: &RegisterArgs, format: OutputFormat) -> Result<()> {
    ...
    std::env::set_var("Isolate_AGENT_ID", &agent_id);
...
pub async fn run_heartbeat(args: &HeartbeatArgs, format: OutputFormat) -> Result<()> {
    let agent_id = std::env::var("Isolate_AGENT_ID").map_err(|_| {
        anyhow::anyhow!("No agent registered. Set Isolate_AGENT_ID or run 'isolate agent register'")
    })?;
    ...
    if result.rows_affected() == 0 {
        anyhow::bail!("Agent '{agent_id}' not found. Agent may have been unregistered. Please run 'isolate agent register' to re-register.");
    }
...
pub async fn run_status(format: OutputFormat) -> Result<()> {
    ...
    let output = if let Some(ref id) = agent_id {
        ...
        AgentStatusOutput {
            registered: false,
            agent: None,
            message: "No agent registered (Isolate_AGENT_ID not set)".to_string(),
        }
    };
...
pub async fn run_unregister(args: &UnregisterArgs, format: OutputFormat) -> Result<()> {
    let agent_id = args
        ...
        .ok_or_else(|| anyhow::anyhow!("No agent ID provided. Set Isolate_AGENT_ID or use --id"))?;
