    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NotInWorkspace { current_location } => write!(
                f,
                "Not in a workspace (currently at: {current_location}). Use 'isolate focus <name>' to switch to a workspace first."
            ),
            Self::NotAJjRepo => write!(f, "Not in a JJ repository. Run 'isolate init' first."),
...
mod tests {
    ...
    fn test_done_output_serialization() {
        let output = DoneOutput {
            workspace_name: "test-ws".to_string(),
            bead_id: Some("isolate-test".to_string()),
            files_committed: 2,
            commits_merged: 1,
            merged: true,
            cleaned: true,
    ...
    fn test_done_display_formats() {
        let preview = DonePreview {
            ...
            bead_to_close: Some("isolate-test".to_string()),
        ...
        assert_eq!(preview.bead_to_close, Some("isolate-test".to_string()));
