...
//! - `isolate schema <response-type>` - Get schema for a response type
//! - `isolate schema --all` - Get all schemas
...
fn run_list(format: OutputFormat) -> Result<()> {
    ...
    let output = SchemaListOutput {
        schemas,
        base_url: "https://isolate.dev/schemas".to_string(),
    };
    ...
    if format.is_json() {
        ...
        println!();
        println!("Use 'isolate schema <name> --json' to get the full JSON Schema.");
    }
...
fn run_single(name: &str, format: OutputFormat) -> Result<()> {
    let schema = match name {
        ...
        "error-response" => get_error_response_schema(),
        _ => {
            return Err(isolate_core::Error::NotFound(format!(
                "Schema '{name}' not found. Use 'isolate schema --list' to see available schemas."
            ))
            .into());
        }
    };
...
fn get_available_schemas() -> Vec<SchemaInfo> {
    vec![
        SchemaInfo {
            ...
            description: "Response from isolate add command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
            name: "remove-response".to_string(),
            description: "Response from isolate remove command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
            name: "list-response".to_string(),
            description: "Response from isolate list command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
            name: "status-response".to_string(),
            description: "Response from isolate status command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
            name: "sync-response".to_string(),
            description: "Response from isolate sync command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
            name: "context-response".to_string(),
            description: "Response from isolate context command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
            name: "ai-status-response".to_string(),
            description: "Response from isolate ai status command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
            name: "ai-next-response".to_string(),
            description: "Response from isolate ai next command".to_string(),
            version: "1.0".to_string(),
        },
        SchemaInfo {
...
fn get_add_response_schema() -> serde_json::Value {
    json!({
        ...
        "$id": "https://isolate.dev/schemas/add-response.v1.json",
        "title": "Add Response",
        "type": "object",
...
fn get_remove_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/remove-response.v1.json",
...
fn get_list_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/list-response.v1.json",
        "title": "List Response",
        "type": "object",
        "required": ["$schema", "_schema_version", "schema_type", "success", "data"],
        "properties": {
...
fn get_status_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/status-response.v1.json",
...
fn get_sync_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/sync-response.v1.json",
        "title": "Sync Response",
        "type": "object",
        "required": ["$schema", "_schema_version", "schema_type", "success", "data"],
        "properties": {
...
fn get_context_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/context-response.v1.json",
...
fn get_ai_status_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/ai-status-response.v1.json",
...
fn get_ai_next_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/ai-next-response.v1.json",
...
fn get_error_response_schema() -> serde_json::Value {
    json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://isolate.dev/schemas/error-response.v1.json",
...
mod tests {
    ...
    mod ai_schema_requirements {
        ...
        /// GIVEN: Any schema
        /// WHEN: Has $id field
        /// THEN: Should point to isolate.dev domain
        #[test]
        fn schema_ids_use_consistent_domain() {
            ...
            for schema in schemas {
                if let Some(id) = schema.get("$id").and_then(|i| i.as_str()) {
                    assert!(
                        id.starts_with("https://isolate.dev/schemas/"),
                        "Schema {id} '{id}' should use isolate.dev domain"
                    );
                }
            }
        }
