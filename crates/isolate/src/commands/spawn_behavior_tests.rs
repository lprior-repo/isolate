...
mod brutal_edge_cases {
    ...
    impl TestRepo {
        ...
        async fn new() -> Result<Self, anyhow::Error> {
            ...
            fs::create_dir_all(root.join(".isolate"))?;
            fs::create_dir_all(root.join(".isolate/workspaces"))?;
            fs::write(
                root.join(".isolate/config.toml"),
                "workspace_dir = \".isolate/workspaces\"\n",
            )?;
...
"#,
    ...
    async fn given_workspace_already_exists_when_spawn_then_handled() {
        ...
        let workspace_path = repo.path().join(".isolate/workspaces/test-bead-1");
    ...
    async fn given_jj_workspace_exists_but_no_db_when_spawn_then_reconciles() {
        ...
        // Create JJ workspace directly (bypassing isolate)
        let workspace_path = repo.path().join(".isolate/workspaces/test-bead-1");
        let _ = fs::create_dir_all(&workspace_path);
    ...
    async fn given_spawn_when_agent_runs_then_env_vars_set() -> Result<(), anyhow::Error> {
        ...
        let _ = fs::write(
            &script_path,
            r#"#!/bin/sh
if [ -z "$Isolate_BEAD_ID" ]; then
    echo "ERROR: Isolate_BEAD_ID not set"
    exit 1
fi
if [ -z "$Isolate_WORKSPACE" ]; then
    echo "ERROR: Isolate_WORKSPACE not set"
    exit 1
fi
if [ -z "$Isolate_ACTIVE" ]; then
    echo "ERROR: Isolate_ACTIVE not set"
    exit 1
fi
...
"#,
        ...
        // Then: Agent receives Isolate_* environment variables
        #[cfg(unix)]
        match result {
            Ok(output) => {
                assert_eq!(
