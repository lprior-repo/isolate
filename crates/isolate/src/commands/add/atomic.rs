            ...
            match (workspace_cleanup, database_cleanup) {
                (Ok(()), Ok(())) => {
                    ...
                    Err(workspace_error).context(format!(
                        "Failed to create workspace, rolled back. Recovery: run 'isolate remove {name} --force' if stale artifacts remain"
                    ))
                }
                (Err(workspace_rollback_error), Ok(())) => {
                    ...
                    Err(workspace_error).context(format!(
                        "Failed to create workspace and workspace rollback failed: {workspace_rollback_error}. Recovery: run 'isolate remove {name} --force'"
                    ))
                }
                (Ok(()), Err(database_rollback_error)) => {
                    ...
                    Err(workspace_error).context(format!(
                        "Failed to create workspace and database rollback failed: {database_rollback_error}. Recovery: run 'isolate remove {name} --force'"
                    ))
                }
                (Err(workspace_rollback_error), Err(database_rollback_error)) => {
                    ...
                    Err(workspace_error).context(format!(
                        "Failed to create workspace, workspace rollback failed: {workspace_rollback_error}, database rollback failed: {database_rollback_error}. Recovery: run 'isolate remove {name} --force'"
                    ))
                }
            }
        }
...
pub(super) async fn rollback_partial_state(
    ...
    let forget_result = isolate_core::jj::workspace_forget(name).await;
    ...
    match tokio::fs::remove_dir_all(workspace_dir).await {
        ...
        Err(cleanup_err) => {
            ...
            Err(anyhow::anyhow!(
                "Failed to rollback partial workspace '{}': {}. Recovery: run 'isolate remove {} --force'",
                workspace_path.display(),
                cleanup_err,
                name
            ))
...
async fn create_jj_workspace(
    ...
    isolate_core::jj_operation_sync::create_workspace_synced(name, workspace_path, repo_root)
        .await
