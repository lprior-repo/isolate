//! Initialize Isolate - sets up everything needed
    ...
    fn acquire(lock_path: PathBuf) -> Result<Self> {
        ...
        if let Ok(metadata) = std::fs::symlink_metadata(&lock_path) {
            if metadata.is_symlink() {
                bail!(
                    "Security: {} is a symlink. Remove it and re-run isolate init.",
                    lock_path.display()
                );
            }
        }
        ...
        file.lock_exclusive().with_context(|| {
            format!(
                "Another isolate init is in progress. If this is incorrect, remove {} and retry.",
                lock_path.display()
            )
        })?;
...
/// 3. Creates the .isolate directory structure:
///    - .isolate/config.toml (default configuration)
///    - .isolate/state.db (sessions database)
///    - .isolate/layouts/ (layouts directory)
...
pub async fn run_with_cwd_and_options(cwd: Option<&Path>, options: InitOptions) -> Result<()> {
    ...
    if options.dry_run {
        println!("Would initialize Isolate in {}", cwd.display());
        println!("Would create .isolate directory structure:");
        println!("  - .isolate/");
        println!("  - .isolate/config.toml");
        println!("  - .isolate/state.db");
        println!("  - .isolate/layouts/");
    ...
    let isolate_dir = root.join(".isolate");

    // Create .isolate directory early so lock file can be created
    // This must happen before acquiring the lock
    if !tokio::fs::try_exists(&isolate_dir).await.unwrap_or(false) {
        tokio::fs::create_dir_all(&isolate_dir)
            .await
            .context("Failed to create .isolate directory")?;
    }

    // Acquire init lock to prevent concurrent initialization
    let lock_path = isolate_dir.join(".init.lock");
    let _lock = InitLock::acquire(lock_path)?;

    // Define paths for all essential files
    let config_path = isolate_dir.join("config.toml");
    let layouts_dir = isolate_dir.join("layouts");
    let db_path = isolate_dir.join("state.db");
    ...
    if is_fully_initialized {
        let response = InitResponse {
            message: "isolate already initialized in this repository.".to_string(),
        ...
        if options.format.is_json() {
            ...
            println!("isolate already initialized in this repository.");
        }
    ...
    // Create .jjignore to prevent .isolate tracking (avoids nested .jj conflicts)
    create_jjignore(&root).await?;

    // Create JJ hooks to enforce isolate workflow (agents can't bypass with --no-verify)
    ...
    if options.format.is_json() {
        ...
        println!("Initialized isolate in {}", root.display());
        println!("  Data directory: .isolate/");
        println!("  Configuration: .isolate/config.toml");
        println!("  State database: .isolate/state.db");
        println!("  Layouts: .isolate/layouts/");
    }
