...
pub fn detect_location(root: &PathBuf) -> Result<Location> {
    ...
    let workspaces_dir = root.join(".isolate/workspaces");
...
async fn get_beads_context() -> Result<Option<BeadsContext>> {
    ...
    if !tokio::fs::try_exists(beads_dir).await.is_ok_and(|e| e) {
        return Ok(None);
    }

    let beads_db = beads_dir.join("issues.jsonl");
    if !tokio::fs::try_exists(&beads_db).await.is_ok_and(|e| e) {
        return Ok(None);
    }
...
async fn check_health(
    ...
    let db_path = root.join(".isolate/state.db");
    if !tokio::fs::try_exists(&db_path).await.is_ok_and(|e| e) {
        errors.push("Session database not found".to_string());
    }

    if let Some(session) = session_context {
        let workspace_path = root.join(".isolate/workspaces").join(&session.name);
...
fn generate_suggestions(
    ...
    match location {
        Location::Main => {
            suggestions.push("Use 'isolate add <name>' to create a workspace".to_string());
        }
