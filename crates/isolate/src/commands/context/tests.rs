...
fn test_context_output_workspace_location() {
    let context = ContextOutput {
        location: Location::Workspace {
            ...
            path: "/home/user/project/.isolate/workspaces/feature-auth".to_string(),
        },
        ..sample_context()
    };
...
fn test_session_context_with_bead() {
    let session = SessionContext {
        ...
        bead_id: Some("isolate-abc123".to_string()),
        agent: None,
        created_at: Utc::now(),
        last_synced: Some(Utc::now()),
    };
    assert_eq!(session.name, "feature-auth");
    assert_eq!(session.bead_id, Some("isolate-abc123".to_string()));
...
fn test_beads_context_active() {
    let beads = BeadsContext {
        active: Some("isolate-abc".to_string()),
        blocked_by: vec![],
        ready_count: 5,
        in_progress_count: 1,
    };
    assert_eq!(beads.active, Some("isolate-abc".to_string()));
...
fn test_beads_context_blocked() {
    let beads = BeadsContext {
        active: None,
        blocked_by: vec!["isolate-123".to_string(), "isolate-456".to_string()],
        ready_count: 3,
        in_progress_count: 0,
    };
...
fn test_suggestions_for_main_location() {
    ...
    assert!(suggestions.iter().any(|s| s.contains("isolate add")));
}
