...
//!
//! This command checks the health of the isolate system and can
...
async fn check_for_recent_recovery() -> Option<String> {
    let log_path = Path::new(".isolate/recovery.log");
...
async fn check_pending_add_operations() -> DoctorCheck {
    let Ok(db) = get_session_db().await else {
        return DoctorCheck {
            ...
            suggestion: Some("Run 'isolate init' and retry doctor".to_string()),
            auto_fixable: false,
            details: None,
        };
    };
    ...
    let pending_count = match pending_add_operation_count(&db).await {
        ...
        Err(error) => {
            return DoctorCheck {
                ...
                suggestion: Some("Run 'isolate doctor --fix' to retry reconciliation".to_string()),
                auto_fixable: true,
                details: None,
            }
        }
    ...
    if pending_count == 0 {
        ...
        DoctorCheck {
            ...
            suggestion: Some(
                "Run 'isolate doctor --fix' to reconcile pending add operations".to_string(),
            ),
            auto_fixable: true,
    ...
    let config = match load_config_or_error().await {
        ...
        Err(e) => {
            return DoctorCheck {
                ...
                suggestion: Some("Check .isolate/config.toml for errors".to_string()),
                auto_fixable: false,
                details: None,
            };
        }
        ...
        .await
    {
        ...
        Err(error) => {
            return DoctorCheck {
                ...
                suggestion: Some("Run 'isolate doctor --fix' to retry validation".to_string()),
                auto_fixable: false,
                details: None,
            };
        }
    ...
    let results = match validator.validate_all(&names).await {
        ...
        Err(e) => {
            return DoctorCheck {
                ...
                suggestion: Some(
                    "Run 'isolate integrity validate <workspace>' for details".to_string(),
                ),
                auto_fixable: false,
                details: None,
            };
...
fn create_fail_check_with_details(details: &[serde_json::Value]) -> DoctorCheck {
    ...
    DoctorCheck {
        ...
        suggestion: Some(
            "Run 'isolate doctor --fix' to attempt recovery (or use 'isolate integrity repair --rebind <name>' when workspaces move)".to_string(),
        ),
        auto_fixable: true,
...
async fn check_jj_repo() -> DoctorCheck {
    ...
    DoctorCheck {
        ...
        suggestion: if is_repo {
            ...
            Some("Initialize JJ: isolate init or jj git init".to_string())
        },
        auto_fixable: false,
        details: None,
    }
...
/// Check workspace context - warn if in a isolate workspace
///
...
fn check_workspace_context() -> DoctorCheck {
    ...
    let in_workspace = current_dir.as_ref().is_some_and(|p| {
        ...
        as_text.contains(".isolate/workspaces") || as_text.contains("/workspaces/")
    });

    // Extract workspace name from .../.isolate/workspaces/<workspace>/...
    let bead_id = current_dir.as_ref().and_then(|path| {
        ...
        parts
            .windows(3)
            .find(|window| window[0] == ".isolate" && window[1] == "workspaces")
    ...
    DoctorCheck {
        ...
        message: if in_workspace {
            ...
            format!("In isolate workspace{suffix}")
        } else {
            "Not in a isolate workspace".to_string()
        },
        ...
        details: in_workspace.then(|| {
            serde_json::json!({
                "location": current_dir
                    ...
                    .map(|p| p.display().to_string())
                    .map_or(String::new(), |value| value),
                "isolate_bead_id": std::env::var("Isolate_BEAD_ID").unwrap_or_else(|_| "<not set>".to_string()),
                "isolate_workspace": std::env::var("Isolate_WORKSPACE").unwrap_or_else(|_| "<not set>".to_string()),
            })
        }),
    }
}
...
async fn check_initialized() -> DoctorCheck {
    // Check for .isolate directory existence directly, without depending on JJ installation
    let isolate_dir = std::path::Path::new(".isolate");
    let config_file = isolate_dir.join("config.toml");
    let initialized = tokio::fs::try_exists(isolate_dir).await.is_ok_and(|v| v)
        && tokio::fs::try_exists(&config_file).await.is_ok_and(|v| v);

    DoctorCheck {
        name: "isolate Initialized".to_string(),
        ...
        message: if initialized {
            ".isolate directory exists with valid config".to_string()
        } else {
        ...
        suggestion: if initialized {
            ...
            Some("Initialize isolate: isolate init".to_string())
        },
        auto_fixable: false,
        details: None,
    }
...
async fn check_state_db() -> DoctorCheck {
    ...
    let db_path = std::path::Path::new(".isolate/state.db");
...
async fn run_integrity_after_recovery() -> Option<DoctorCheck> {
    ...
    Some(DoctorCheck {
        ...
        suggestion: Some(
            "Run 'isolate integrity repair --rebind' to fix relocated workspaces".to_string(),
        ),
        auto_fixable: false,
        details: Some(serde_json::json!({
...
fn create_recovery_check(recovery_info: &str) -> DoctorCheck {
    DoctorCheck {
        ...
        suggestion: Some("Recovery completed. Review .isolate/recovery.log for details and run 'isolate backup --create' to capture the recovered state.".to_string()),
        auto_fixable: false,
        details: Some(serde_json::json!({
            "recovered": true,
            "details": recovery_info
...
async fn check_db_file_exists(db_path: &Path) -> Result<std::fs::Metadata, DoctorCheck> {
    if !tokio::fs::try_exists(db_path).await.is_ok_and(|v| v) {
        return Err(DoctorCheck {
            ...
            suggestion: Some("Run 'isolate init' to create database".to_string()),
            auto_fixable: false,
            details: None,
        });
    }
...
async fn check_db_readable(db_path: &Path) -> Result<(), DoctorCheck> {
    tokio::fs::File::open(db_path)
        ...
        .map_err(|e| DoctorCheck {
            ...
            suggestion: Some("Check file permissions on .isolate/state.db".to_string()),
            auto_fixable: false,
            details: Some(serde_json::json!({
...
async fn check_db_file_size_and_integrity(file_size: u64, db_path: &Path) -> DoctorCheck {
    ...
    if file_size == 0 || file_size < 100 {
        return DoctorCheck {
            ...
            suggestion: Some(
                "Database may be corrupted. Run 'isolate doctor --fix' to attempt recovery."
                    .to_string(),
            ),
            auto_fixable: true,
            details: Some(serde_json::json!({
    ...
    match integrity_result {
        ...
        Err(integrity_error) => {
            ...
            DoctorCheck {
                ...
                suggestion: Some(
                    "Database is corrupted. Run 'isolate doctor --fix' to attempt recovery."
                        .to_string(),
                ),
                auto_fixable: true,
                details: Some(serde_json::json!({
...
async fn check_orphaned_workspaces() -> DoctorCheck {
    ...
    if total_orphans == 0 {
        ...
        DoctorCheck {
            ...
            suggestion: Some("Run 'isolate doctor --fix' to clean up".to_string()),
            auto_fixable: true,
...
async fn check_stale_sessions() -> DoctorCheck {
    ...
    if stale_sessions.is_empty() {
        ...
        DoctorCheck {
            ...
            suggestion: Some(
                "Check for interrupted operations or run 'isolate remove <name>' to clean up"
                    .to_string(),
            ),
            auto_fixable: false,
...
async fn check_workflow_violations() -> DoctorCheck {
    ...
    let on_main = current_dir
        .as_ref()
        .map(|p| !p.to_string_lossy().contains(".isolate/workspaces"))
        .is_none_or(|v| v);
    ...
    if on_main && !active_sessions.is_empty() {
        ...
        return DoctorCheck {
            ...
            suggestion: Some(format!(
                "Work should happen in isolated workspaces. Run: isolate attach {}",
                session_names
                    .first()
                    .map(String::as_str)
                    .map_or("<name>", |v| v)
    ...
    DoctorCheck {
        ...
        suggestion: if active_sessions.is_empty() {
            Some("Start work: isolate spawn <bead-id>".to_string())
        } else {
            None
        },
        auto_fixable: false,
...
fn fix_pending_add_operations(check: &DoctorCheck, dry_run: bool) -> Result<String, String> {
    let pending = check
        .details
        .as_ref()
...
async fn fix_workspace_integrity(check: &DoctorCheck, dry_run: bool) -> Result<String, String> {
    ...
    let executor = isolate_core::workspace_integrity::RepairExecutor::new();
    let db = get_session_db()
        .await
...
mod tests {
    ...
    #[tokio::test]
    #[serial]
    async fn test_check_initialized_detects_isolate_directory() {
        ...
        // Test 1: No .isolate directory - should warn
        let result = check_initialized().await;
        assert_eq!(result.status, CheckStatus::Warn);
        assert_eq!(result.name, "isolate Initialized");
        assert!(result.message.contains("not initialized"));

        // Test 2: .isolate directory exists but no config.toml - should warn
        if tokio::fs::create_dir(".isolate").await.is_err() {
        ...
        // Test 3: .isolate directory with config.toml - should pass
        if tokio::fs::write(".isolate/config.toml", "workspace_dir = \"test\"")
            .await
            .is_err()
        {
            let _ = std::env::set_current_dir(original_dir);
        ...
        assert!(result.message.contains(".isolate directory exists"));
    ...
    async fn test_check_initialized_independent_of_jj() {
        ...
        // Create .isolate structure WITHOUT initializing a JJ repo
        if tokio::fs::create_dir(".isolate").await.is_err() {
            let _ = std::env::set_current_dir(original_dir);
            return;
        }
        ...
        // Even without JJ installed/initialized, should detect .isolate
        let result = check_initialized().await;
        assert_eq!(result.status, CheckStatus::Pass);
    ...
    async fn test_check_jj_installed_vs_check_initialized() {
        ...
        assert_eq!(init_check.name, "isolate Initialized");
