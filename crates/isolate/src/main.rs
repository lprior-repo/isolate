...
use isolate_core::ShutdownCoordinator;

/// Install panic hook to handle broken pipe gracefully
///
/// When piping output (e.g., `isolate list | head -n 5`), if the receiving end
...
async fn main() {
    ...
    // AI agents that don't have JJ cannot use isolate - period.
    ...
    if !cli::is_jj_installed().await {
        #[allow(clippy::print_stderr)]
        {
            ...
            eprintln!("║  🔒 Isolate REQUIRES JJ (JUJUTSU)                                          ║");

            eprintln!("╚════════════════════════════════════════════════════════════════════════╝");

            eprintln!();

            eprintln!("JJ is NOT installed. Isolate cannot function without it.");
            ...
            eprintln!("Isolate is built on top of JJ for workspace isolation.");
    ...
    let (mut sigint, mut sigterm) = match isolate_core::signal_channels().await {
