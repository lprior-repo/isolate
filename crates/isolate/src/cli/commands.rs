    ...
    ClapCommand::new("agents")
        ...
        .subcommand(
            ClapCommand::new("register")
                ...
                .long_about(
                    "Register this process as an agent for isolate tracking.


                    Stores agent identity in isolate and prints the Isolate_AGENT_ID value to export in your shell.
        ...
        .subcommand(
            ClapCommand::new("heartbeat")
                ...
                .long_about(
                    ...
                    Requires Isolate_AGENT_ID to be set (run 'isolate agent register' first).",
                )
                .arg(
                    Arg::new("command")
                        .long("command")
        ...
        .subcommand(
            ClapCommand::new("status")
                ...
                .long_about(
                    ...
                    Uses Isolate_AGENT_ID environment variable.",
                ),
        )
        .subcommand(
            ClapCommand::new("unregister")
                ...
                .long_about(
                    "Remove this agent from isolate tracking.


                    Clears Isolate_AGENT_ID environment variable.",
                )
                .arg(
                    Arg::new("id")
                        .long("id")
                        .value_name("AGENT_ID")
                        .help("Agent ID to unregister (uses Isolate_AGENT_ID if not provided)"),
                ),
        )
        .after_help(after_help_text(
            &[
                "isolate agents                      List all active agents",
                "isolate agents --all                Include stale agents",
                "isolate agents --session work       Filter by session",
                "isolate agents register             Register as an agent",
                "isolate agents heartbeat -c build   Send heartbeat with command",
                "isolate agents unregister           Unregister current agent",
            ],
            None,
        ))
}
...
pub fn cmd_broadcast() -> ClapCommand {
    ClapCommand::new("broadcast")
        ...
        .after_help(after_help_text(
            &[
                "isolate broadcast 'Hello, team!'     Send message to all agents",
                "isolate broadcast 'Work complete'    Notify other agents",
                "isolate broadcast --json             Output in JSON format",
            ],
            None,
        ))
        .arg(
        ...
        .arg(
            Arg::new("agent-id")
                .long("agent-id")
                .value_name("AGENT_ID")
                .help("Agent ID of the sender (uses Isolate_AGENT_ID if not provided)"),
        )
        .arg(
            Arg::new("json")
                .long("json")
...
pub fn build_cli() -> ClapCommand {
    ClapCommand::new("isolate")
        ...
        .about("Isolate - Isolated workspace manager combining JJ workspaces with Zellij sessions")
        .long_about(
            "Isolate creates isolated JJ workspaces paired with Zellij tabs for parallel work.\n\n\
            Core workflow:\n  \
              isolate init          Initialize isolate in a JJ repo\n  \
              isolate add <name>    Create session for manual work (you control tab)\n  \
              isolate spawn <bead>  Create session for automated agent work\n  \
            isolate focus <name>  Switch Zellij tab (inside Zellij)\n  \
            isolate done          Merge workspace to main and clean up",
        )
