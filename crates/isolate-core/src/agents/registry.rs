    ...
    pub fn into_error(self) -> Error {
        match self {
            ...
            Self::ReservedKeyword(keyword) => Error::ValidationError {
                message: format!("Agent ID '{}' is a reserved keyword", keyword),
                field: Some("agent_id".to_string()),
                value: Some(keyword),
                constraints: vec!["not a reserved keyword".to_string()],
            },
            Self::TooLong(max) => Error::ValidationError {
                message: format!("Agent ID exceeds maximum length of {} characters", max),
                field: Some("agent_id".to_string()),
                value: None,
                constraints: [format!("max {} characters", max)].to_vec(),
            },
        }
    }
}
