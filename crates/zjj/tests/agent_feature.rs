...
impl AgentTestContext {
    ...
    /// Run zjj command and store result
    pub async fn run_zjj(&self, args: &[&str]) -> CommandResult {
        let result = self.harness.zjj(args);
        *self.last_result.lock().await = Some(result.clone());
        result
    }

    /// Run zjj command with environment variables
    pub async fn run_zjj_with_env(
        &self,
        args: &[&str],
        env_vars: &[(&str, &str)],
    ) -> CommandResult {
        let result = self.harness.zjj_with_env(args, env_vars);
        *self.last_result.lock().await = Some(result.clone());
        result
    }
}
...
async fn agent_exists_in_db(ctx: &AgentTestContext, agent_id: &str) -> Result<bool> {
    let result = ctx.run_zjj(&["agents", "--all", "--json"]).await;
...
async fn scenario_register_creates_agent() {
    ...
    // WHEN
    let result = ctx
        .run_zjj(&["agents", "register", "--id", "agent-test-001", "--json"])
        .await;
...
async fn scenario_register_auto_generated_id() {
    ...
    // WHEN
    let result = ctx.run_zjj(&["agents", "register", "--json"]).await;
...
async fn scenario_duplicate_id_succeeds_with_update() {
    ...
    // Create first agent
    let _ = ctx
        .run_zjj(&["agents", "register", "--id", "agent-duplicate", "--json"])
        .await;
...
async fn scenario_register_invalid_id_fails() {
    ...
    let result = ctx
        .run_zjj(&["agents", "register", "--id", "", "--json"])
        .await;
...
async fn scenario_register_reserved_keyword_fails() {
    ...
    let result = ctx
        .run_zjj(&["agents", "register", "--id", "null", "--json"])
        .await;
...
/// AND: the environment variable `"ZJJ_AGENT_ID"` is set
...
async fn scenario_heartbeat_updates_timestamp() {
    ...
    // Register agent
    let _ = ctx
        .run_zjj(&["agents", "register", "--id", "agent-heartbeat", "--json"])
        .await;

    // WHEN - Send heartbeat with ZJJ_AGENT_ID set
    let result = ctx
        .run_zjj_with_env(
            &["agents", "heartbeat", "--json"],
            &[("ZJJ_AGENT_ID", "agent-heartbeat")],
        )
        .await;
...
/// THEN: the agent should have `current_command` set to "zjj add feature-x"
#[tokio::test]
async fn scenario_heartbeat_with_command() {
    ...
    // Register agent
    let _ = ctx
        .run_zjj(&["agents", "register", "--id", "agent-cmd", "--json"])
        .await;
...
async fn scenario_unknown_agent_heartbeat_fails() {
    ...
    // WHEN - Attempt heartbeat without ZJJ_AGENT_ID
    // Note: We need to clear the env var from any previous registration
    let result = ctx
        .run_zjj_with_env(&["agents", "heartbeat", "--json"], &[("ZJJ_AGENT_ID", "")])
        .await;
...
async fn scenario_whoami_returns_identity() {
    ...
    // Register agent
    let _ = ctx
        .run_zjj(&["agents", "register", "--id", "agent-whoami", "--json"])
        .await;

    // WHEN
    let result = ctx
        .run_zjj_with_env(&["whoami", "--json"], &[("ZJJ_AGENT_ID", "agent-whoami")])
        .await;
...
async fn scenario_whoami_unregistered() {
    ...
    // WHEN - Query whoami using regular zjj (which may or may not have ZJJ_AGENT_ID set)
    // The key test is that whoami succeeds and returns valid JSON
    let result = ctx.run_zjj(&["whoami", "--json"]).await;
...
async fn scenario_list_shows_all_agents() {
    ...
    for agent_id in &["agent-alpha", "agent-beta", "agent-gamma"] {
        let _ = ctx
            .run_zjj(&["agents", "register", "--id", agent_id, "--json"])
            .await;
        ctx.track_agent(agent_id).await;
    }
...
async fn scenario_list_empty() {
    ...
    // WHEN
    let result = ctx.run_zjj(&["agents", "--json"]).await;
...
async fn scenario_status_shows_details() {
    ...
    // Register agent
    let _ = ctx
        .run_zjj(&["agents", "register", "--id", "agent-status", "--json"])
        .await;
...
async fn scenario_unregister_removes_agent() {
    ...
    // Register agent
    let _ = ctx
        .run_zjj(&["agents", "register", "--id", "agent-unregister", "--json"])
        .await;
