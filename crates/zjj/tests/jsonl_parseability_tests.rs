...
//!
//! These tests verify that all zjj commands produce valid JSONL output
...
fn test_output_has_consistent_schema() {
    ...
    let result1 = harness.zjj(&["list"]);
    let result2 = harness.zjj(&["list"]);
...
fn test_session_output_has_required_fields() {
    ...
    // Cleanup
    let _ = harness.zjj(&["remove", "test-session-fields", "--merge"]);
}
...
fn test_result_output_has_required_fields() {
    ...
    // Cleanup
    let _ = harness.zjj(&["remove", "test-result-fields", "--merge"]);
}
...
fn test_result_output_is_final_line_for_add() {
    ...
    // Cleanup
    let _ = harness.zjj(&["remove", "test-final-result", "--merge"]);
}
...
fn test_all_output_variants_have_correct_structure() {
    ...
    // Cleanup
    let _ = harness.zjj(&["remove", "test-action-variant", "--merge"]);
}
...
fn test_enums_serialize_to_snake_case() {
    ...
    // Cleanup
    let _ = harness.zjj(&["remove", "test-enum-serialization", "--merge"]);
}
