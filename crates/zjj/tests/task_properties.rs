//! Property-Based Tests for Task Object Invariants
//!
//! This module defines property tests using proptest that specify the expected
//! invariants for the task object. These tests are in the RED phase and MUST
//! FAIL initially since the implementation does not exist yet.
//!
//! ## Properties Tested
//!
//! 1. **Lock Exclusivity**: No two agents can own the same task simultaneously
//! 2. **TTL Expiration**: Expired locks are automatically released
//! 3. **State Transitions**: All state transitions follow the valid state machine
//! 4. **Concurrent Claims**: Exactly one claim succeeds when multiple agents try
//!
//! ## Running Tests
//!
//! ```bash
//! cargo test --test task_properties
//! ```
//!
//! To reproduce failures with a specific seed:
//!
//! ```bash
//! PROPTEST_SEED=0x123456789abcdef0 cargo test --test task_properties
//! ```
//!
//! ## RED Phase Status
//!
//! These tests are designed to FAIL until the real `TaskObject` implementation
//! is provided in zjj-core. The stub implementation here is intentionally
//! incomplete to demonstrate the expected failure modes.
...
// ═══════════════════════════════════════════════════════════════════════════
// TASK DOMAIN TYPES (Stubs - implementation will be in zjj-core)
// ═══════════════════════════════════════════════════════════════════════════
