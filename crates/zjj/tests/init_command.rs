//! Modular tests for `jjz init` command
//!
...
fn test_init_creates_state_db() {
    tokio_test::block_on(async {
        ...
        let result: Result<(i64,), _> = sqlx::query_as("SELECT COUNT(*) FROM sessions")
            .fetch_one(&mut conn)
            .await;
...
fn test_init_state_db_has_correct_schema() {
    tokio_test::block_on(async {
        ...
        let Ok(rows) = sqlx::query_as::<_, ColumnInfo>("PRAGMA table_info(sessions)")
            .fetch_all(&mut conn)
            .await
        else {
            std::process::abort()
        ...
        let columns: Vec<String> = rows.into_iter().map(|info| info.name).collect();
...
fn test_init_creates_indexes() {
    tokio_test::block_on(async {
        ...
        let Ok(rows) = sqlx::query_as::<_, (String,)>(
        ...
        let indexes: Vec<String> = rows.into_iter().map(|(name,)| name).collect();
