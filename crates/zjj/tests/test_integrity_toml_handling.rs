...
async fn test_integrity_repair_handles_bad_toml_gracefully() -> Result<()> {
    ...
    tokio::fs::create_dir_all(&workspace_path)
        .await
        .map_err(|e| zjj_core::Error::IoError(format!("Failed to create workspace: {e}")))?;

    // Create a corrupted .zjj directory with bad TOML
    let zjj_dir = workspace_path.join(".zjj");
...
"#;
    ...
    tokio::fs::create_dir_all(&jj_dir)
        .await
        .map_err(|e| zjj_core::Error::IoError(format!("Failed to create JJ dir: {e}")))?;
    tokio::fs::write(jj_dir.join("op1"), "test")
        .await
        .map_err(|e| zjj_core::Error::IoError(format!("Failed to write op file: {e}")))?;
