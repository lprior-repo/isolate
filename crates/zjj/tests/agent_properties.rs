//! Property-Based Tests for Agent Object Invariants
//!
//! This module defines property tests using proptest that specify the expected
//! invariants for the agent object. These tests are in the GREEN phase and all
//! pass with the current implementation.
//!
//! ## Properties Tested
//!
//! 1. **ID Uniqueness**: All agent IDs in a collection are unique
//! 2. **Heartbeat Timing**: Heartbeats update `last_seen` timestamps correctly
//! 3. **Stale Detection**: Agents beyond timeout are correctly identified as stale
//! 4. **Session Binding**: An agent can only be bound to one session at a time
//! 5. **State Machine**: Agent status transitions follow the state machine rules
//! 6. **Combined Invariants**: All invariants hold together under any operation sequence
//!
//! ## Running Tests
//!
//! ```bash
//! cargo test --test agent_properties
//! ```
//!
//! To reproduce failures with a specific seed:
//!
//! ```bash
//! PROPTEST_SEED=0x123456789abcdef0 cargo test --test agent_properties
//! ```
//!
//! ## GREEN Phase Status
//!
//! All tests pass. The implementation correctly enforces:
//! - ID uniqueness on registration
//! - Heartbeat timing and staleness detection (using <= for boundary)
//! - Session binding exclusivity through the registry
...
// ═══════════════════════════════════════════════════════════════════════════
// AGENT DOMAIN TYPES (Stubs - implementation will be in zjj-core)
// ═══════════════════════════════════════════════════════════════════════════
...
/// The real implementation will be provided in zjj-core and should:
...
proptest! {
    ...
    fn adv_large_timestamps_no_overflow(
        agent_id in agent_id_strategy(),
        heartbeat_time in 1700000000_u64..u64::MAX - 3600,
