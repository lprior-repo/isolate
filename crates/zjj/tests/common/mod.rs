...
impl TestHarness {
    ...
    /// This reads the config to determine the actual workspace directory
    pub fn workspace_path(&self, session: &str) -> PathBuf {
        // Try to read the workspace_dir from config, fall back to default pattern
        let config_path = self.jjz_dir().join("config.toml");
        if let Ok(content) = std::fs::read_to_string(&config_path) {
            // Parse TOML to get workspace_dir
            if let Ok(config) = content.parse::<toml::Value>() {
                if let Some(workspace_dir) = config.get("workspace_dir").and_then(|v| v.as_str()) {
                    // Resolve the workspace_dir pattern
                    let resolved = self.resolve_workspace_dir(workspace_dir);
                    return PathBuf::from(resolved).join(session);
                }
            }
        }

        // Fall back to default: ../{repo}__workspaces
        let repo_name = self
            .repo_path
            .file_name()
            .and_then(|n| n.to_str())
    ...
    /// Resolve `workspace_dir` pattern (replace {repo} placeholder)
    fn resolve_workspace_dir(&self, pattern: &str) -> String {
        let repo_name = self
            .repo_path
            .file_name()
            .and_then(|n| n.to_str())
        ...
        let resolved = pattern.replace("{repo}", repo_name);
