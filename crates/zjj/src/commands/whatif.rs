    ...
    if options.format.is_json() {
        ...
        if !result.creates.is_empty() {
            ...
            for c in &result.creates {
                println!("  + [{}] {}: {}", c.resource_type, c.resource, c.description);
            }
            println!();
        }
        ...
        if !result.modifies.is_empty() {
            ...
            for m in &result.modifies {
                println!("  ~ [{}] {}: {}", m.resource_type, m.resource, m.description);
            }
            println!();
        }
        ...
        if !result.deletes.is_empty() {
            ...
            for d in &result.deletes {
                println!("  - [{}] {}: {}", d.resource_type, d.resource, d.description);
            }
            println!();
        }
...
fn preview_work(args: &[String]) -> Result<WhatIfResult> {
    ...
    result.side_effects.push("Sets ZJJ_AGENT_ID in environment".to_string());
...
fn preview_sync(args: &[String]) -> Result<WhatIfResult> {
    ...
    Ok(WhatIfResult {
        ...
        deletes: vec![],
        side_effects: vec![
            "Commit history may be rewritten".to_string(),
        ],
        reversible: true,
