...
impl WorkerError {
    ...
    ///
    /// This classification is based on the error type:
    /// - Retryable: transient errors that may succeed on retry
    /// - Terminal: permanent errors that will always fail
    #[must_use]
    pub const fn classify(&self) -> ErrorClass {
        match self {
            ...
            // Git operations may be retryable depending on context
...
///
/// This function analyzes error messages to determine retryability.
/// It uses pattern matching on common error signatures.
#[must_use]
pub fn classify_error_message(error_msg: &str) -> ErrorClass {
    let lower = error_msg.to_lowercase();

    // Terminal patterns - these indicate permanent failures
    ...
    // Retryable patterns - these indicate transient failures
    ...
    // Check terminal patterns first (higher priority)
...
///
/// Even retryable errors become terminal if max_attempts is exceeded.
///
/// # Arguments
/// * `error_msg` - The error message to classify
/// * `attempt_count` - Current number of attempts made
/// * `max_attempts` - Maximum allowed attempts
///
/// # Returns
/// `ErrorClass::Terminal` if max_attempts exceeded, otherwise classifies the error.
#[must_use]
pub fn classify_with_attempts(
    ...
    // If max attempts exceeded, always terminal
    if attempt_count >= max_attempts {
        return ErrorClass::Terminal;
    }

    // Otherwise, classify based on error type
    classify_error_message(error_msg)
}

/// Determine if an error should be retried based on classification and attempts.
///
/// # Arguments
/// * `error_msg` - The error message to classify
/// * `attempt_count` - Current number of attempts made
...
///
/// This function analyzes the error chain and message to produce
/// an appropriate WorkerError variant.
#[must_use]
pub fn from_anyhow(error: &anyhow::Error) -> WorkerError {
    let error_str = error.to_string();
    let lower = error_str.to_lowercase();

    // Check chain for specific error types
    let chain: Vec<String> = error.chain().map(std::string::ToString::to_string).collect();
...
///
/// # Returns
/// `ErrorClass` indicating retryability.
#[must_use]
