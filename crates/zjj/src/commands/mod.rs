...
//!
//! This module contains all command implementations organized into submodules and routers:
//! - Individual command modules: add, remove, list, focus, status, etc.
//! - Routers: session, utility, introspection (group commands by logical category)
...
// Command routers - organize commands into logical groups
pub mod routers;
...
mod tests {
    ...
    fn test_zjj_data_dir_without_jj() {
        ...
        if let Err(e) = result {
            ...
            assert!(
                !msg.contains("JJ is not installed"),
                "zjj_data_dir should NOT check for jj installation, got: {msg}"
            );
        } else {
