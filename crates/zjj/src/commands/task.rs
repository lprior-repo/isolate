...
impl TaskRepository {
    ...
    pub fn new(root: &str) -> Self {
        Self {
            ...
            data_dir: PathBuf::from(root).join(".zjj"),
        }
    }
...
fn get_agent_id() -> String {
    std::env::var("ZJJ_AGENT_ID").unwrap_or_else(|_| format!("agent-{}", std::process::id()))
}
...
pub async fn handle_task_claim(args: &clap::ArgMatches) -> Result<()> {
    ...
    let ttl_seconds: u64 = std::env::var("ZJJ_TASK_TTL")
        .ok()
...
pub async fn handle_task_start(args: &clap::ArgMatches) -> Result<()> {
    ...
    let result = serde_json::json!({
        "task_id": task_id,
        "status": "started",
        "workspace": format!(".zjj/workspaces/{}", task_id),
        "template": template,
    });
    ...
    if format.is_json() {
        ...
        println!("  Workspace: .zjj/workspaces/{task_id}");
        if let Some(t) = template {
            println!("  Template: {t}");
        }
        println!();
        println!("Run 'zjj spawn {task_id}' to begin work");
    }
...
pub async fn handle_task_done(args: &clap::ArgMatches) -> Result<()> {
    ...
    let task_id = match task_id {
        ...
        None => {
            ...
            std::env::var("ZJJ_BEAD_ID")
                .context("No task ID provided and not in a workspace (ZJJ_BEAD_ID not set)")?
        }
    };
...
pub async fn handle_task(args: &clap::ArgMatches) -> Result<()> {
    match args.subcommand() {
        ...
        _ => {
            ...
            println!();
            println!("  zjj task list [--all] [--state <STATE>]  List tasks");
            println!("  zjj task show <ID>                        Show task details");
            println!("  zjj task claim <ID>                       Claim a task");
            println!("  zjj task yield <ID>                       Release a claimed task");
            println!("  zjj task start <ID> [--template <T>]      Start work on a task");
            println!("  zjj task done [ID]                        Complete a task");
            println!();
            println!("Run 'zjj task <command> --help' for more information.");
            Ok(())
        }
    }
}
