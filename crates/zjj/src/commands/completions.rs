...
_zjj() {
    ...
    case "${prev}" in
        focus|remove|status|sync|diff|claim|yield|rename|pause|resume|clone)
            local sessions=$(zjj list --json 2>/dev/null | jq -r '.data[].name' 2>/dev/null)
...
_zjj_sessions() {
    local sessions
    sessions=(${(f)"$(zjj list --json 2>/dev/null | jq -r '.data[].name' 2>/dev/null)"})
...
fn generate_fish_completions() -> String {
    r#"# zjj fish completion

# Disable file completion by default
complete -c zjj -f

# Commands
complete -c zjj -n "__fish_use_subcommand" -a init -d "Initialize zjj"
complete -c zjj -n "__fish_use_subcommand" -a add -d "Create session"
complete -c zjj -n "__fish_use_subcommand" -a list -d "List sessions"
complete -c zjj -n "__fish_use_subcommand" -a remove -d "Remove session"
complete -c zjj -n "__fish_use_subcommand" -a focus -d "Switch to session"
complete -c zjj -n "__fish_use_subcommand" -a status -d "Show status"
complete -c zjj -n "__fish_use_subcommand" -a sync -d "Sync with main"
complete -c zjj -n "__fish_use_subcommand" -a done -d "Complete and merge"
complete -c zjj -n "__fish_use_subcommand" -a undo -d "Revert last done"
complete -c zjj -n "__fish_use_subcommand" -a revert -d "Revert specific merge"
complete -c zjj -n "__fish_use_subcommand" -a spawn -d "Spawn agent"
complete -c zjj -n "__fish_use_subcommand" -a work -d "Start working"
complete -c zjj -n "__fish_use_subcommand" -a abort -d "Abandon workspace"
complete -c zjj -n "__fish_use_subcommand" -a agents -d "Manage agents"
complete -c zjj -n "__fish_use_subcommand" -a ai -d "AI entry point"
complete -c zjj -n "__fish_use_subcommand" -a checkpoint -d "Manage checkpoints"
...
complete -c zjj -n "__fish_use_subcommand" -a query -d "Query state"
complete -c zjj -n "__fish_use_subcommand" -a whereami -d "Location query"
complete -c zjj -n "__fish_use_subcommand" -a whoami -d "Identity query"
complete -c zjj -n "__fish_use_subcommand" -a contract -d "Show contracts"
complete -c zjj -n "__fish_use_subcommand" -a examples -d "Show examples"
complete -c zjj -n "__fish_use_subcommand" -a validate -d "Validate inputs"
complete -c zjj -n "__fish_use_subcommand" -a whatif -d "Preview command"
complete -c zjj -n "__fish_use_subcommand" -a claim -d "Claim lock"
complete -c zjj -n "__fish_use_subcommand" -a yield -d "Release lock"
complete -c zjj -n "__fish_use_subcommand" -a events -d "Show events"
complete -c zjj -n "__fish_use_subcommand" -a batch -d "Batch execute"
complete -c zjj -n "__fish_use_subcommand" -a completions -d "Generate completions"

# Session name completion for relevant commands
function __fish_zjj_sessions
    zjj list --json 2>/dev/null | jq -r '.data[].name' 2>/dev/null
end

complete -c zjj -n "__fish_seen_subcommand_from focus remove status sync diff claim yield" -a "(__fish_zjj_sessions)"

# Global flags
complete -c zjj -l json -d "Output as JSON"
complete -c zjj -l help -d "Show help"
"#.to_string()
}

fn generate_powershell_completions() -> String {
    r#"# zjj PowerShell completion
...
Register-ArgumentCompleter -CommandName zjj -ScriptBlock {
    ...
    if ($words.Count -eq 1) {
        # Complete commands
        $script:zjjCommands | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
    ...
    elseif ($words.Count -ge 2) {
        ...
        if ($command -in @('focus', 'remove', 'status', 'sync', 'diff', 'claim', 'yield')) {
            $sessions = zjj list --json 2>$null | ConvertFrom-Json | Select-Object -ExpandProperty data | Select-Object -ExpandProperty name
...
set edit:completion:arg-completer[zjj] = {|@words|
    ...
    if (eq (count $words) 1) {
        ...
        if (has-value [focus remove status sync diff claim yield] $cmd) {
            ...
            try {
                var sessions = (zjj list --json 2>/dev/null | from-json)[data]
                for sess $sessions {
                    put $sess[name]
                }
            } catch { }
...
fn get_install_instructions(shell: Shell) -> String {
    match shell {
        Shell::Bash => "Add to ~/.bashrc:\n  source <(zjj completions bash)".to_string(),
        Shell::Zsh => "Add to ~/.zshrc:\n  source <(zjj completions zsh)\n\nOr save to ~/.zfunc/_zjj".to_string(),
        Shell::Fish => "Save to ~/.config/fish/completions/zjj.fish:\n  zjj completions fish > ~/.config/fish/completions/zjj.fish".to_string(),
        Shell::PowerShell => "Add to $PROFILE:\n  zjj completions powershell | Out-String | Invoke-Expression".to_string(),
        Shell::Elvish => "Save to ~/.elvish/lib/zjj.elv:\n  zjj completions elvish > ~/.elvish/lib/zjj.elv".to_string(),
    }
}
...
mod tests {
    ...
    fn test_generate_bash_completions() {
        ...
        assert!(script.contains("_zjj()"));
        assert!(script.contains("complete -F _zjj zjj"));
    }

    #[test]
    fn test_generate_zsh_completions() {
        let script = generate_zsh_completions();
        assert!(script.contains("#compdef zjj"));
        assert!(script.contains("_zjj()"));
    }
    ...
    fn test_generate_fish_completions() {
        ...
        assert!(script.contains("complete -c zjj"));
    }
