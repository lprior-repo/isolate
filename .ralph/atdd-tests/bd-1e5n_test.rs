...
//!   - WHEN workspace is in queue, THE SYSTEM SHALL display stack_depth, parent_workspace,
//!     children (from get_children), and stack_root
...
fn test_stack_status_json_format() {
    ...
    if success {
        ...
        let parse_result: Result<StackStatusEnvelope, _> =
...
fn test_stack_status_json_has_required_fields() {
    ...
    if success {
        let envelope: StackStatusEnvelope =
...
fn test_stack_status_not_in_queue_error_json() {
    ...
    match error_result {
        ...
        Err(_) => {
            ...
            let status_result: Result<StackStatusEnvelope, _> =
            ...
            match status_result {
                ...
                Err(e) => {
                    panic!(
                        "Expected valid JSON output, got parse error: {e}\nOutput: {combined}"
                    );
                }
            }
        }
