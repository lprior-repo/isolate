...
# Martin Fowler Test Plan: Session Focus

## Happy Path Tests

- test_focus_active_session_from_inside_zellij_switches_tab
  Given: A session named "feature-focus" exists with status "active"
  And: I am inside Zellij (ZELLIJ_SESSION_NAME is set)
  When: I focus on the session "feature-focus"
  Then:
    - The Zellij tab "isolate:feature-focus" should become active
    - The session details should be returned as JSON
    - Action with verb "focus" and status "completed" should be emitted

- test_focus_active_session_from_outside_zellij_attaches
  Given: A session named "feature-attach" exists with status "active"
  And: I am not inside Zellij (ZELLIJ_SESSION_NAME is not set)
  When: I focus on the session "feature-attach"
  Then:
    - Zellij should attach to the session
    - The session details should be returned as JSON
    - Action with verb "focus" and status "completed" should be emitted

## Error Path Tests

- test_focus_with_empty_name_returns_error
  Given: No session name is provided
  When: I attempt to focus on an empty string
  Then: Returns `Err(Error::SessionNameRequired)` with message "Session name is required"

- test_focus_with_whitespace_name_returns_error
  Given: No session name is provided (only whitespace)
  When: I attempt to focus on "   "
  Then: Returns `Err(Error::SessionNameRequired)` with message "Session name is required"

- test_focus_nonexistent_session_returns_not_found
  Given: No session named "missing" exists
  When: I attempt to focus on the session "missing"
  Then: Returns `Err(Error::SessionNotFound)` with message "Session 'missing' not found"

- test_focus_completed_session_returns_invalid_status
  Given: A session named "completed-session" exists with status "completed"
  When: I focus on the session "completed-session"
  Then: Returns `Err(Error::InvalidSessionStatus)` with message "Cannot focus completed session"

- test_focus_paused_session_returns_invalid_status
  Given: A session named "paused-session" exists with status "paused"
  When: I focus on the session "paused-session"
  Then: Returns `Err(Error::InvalidSessionStatus)` with message "Cannot focus session in 'paused' state"

## Edge Case Tests

- test_focus_with_hyphenated_name_succeeds
  Given: A session named "my-test-session" exists with status "active"
  When: I focus on the session "my-test-session"
  Then: Focus succeeds and Zellij tab "isolate:my-test-session" becomes active

- test_focus_with_underscore_name_succeeds
  Given: A session named "my_test_session" exists with status "active"
  When: I focus on the session "my_test_session"
  Then: Focus succeeds and Zellij tab "isolate:my_test_session" becomes active

- test_focus_twice_same_session_succeeds
  Given: A session named "double-focus" exists with status "active"
  And: I have already focused on "double-focus"
  When: I focus on the session "double-focus" again
  Then: Focus succeeds (idempotent operation)

## Contract Verification Tests

- test_precondition_name_not_empty_enforced_at_runtime
  Given: No validation has occurred
  When: run_with_options(Some(""), &FocusOptions::default()) is called
  Then: Returns Error::SessionNameRequired

- test_precondition_name_not_whitespace_only_enforced_at_runtime
  Given: No validation has occurred
  When: run_with_options(Some("   "), &FocusOptions::default()) is called
  Then: Returns Error::SessionNameRequired

- test_precondition_session_exists_enforced_at_runtime
  Given: Database does not contain "nonexistent"
  When: run_with_options(Some("nonexistent"), &FocusOptions::default()) is called
  Then: Returns Error::SessionNotFound

- test_precondition_session_status_active_enforced_at_runtime
  Given: A session "completed-session" exists with status Completed
  When: run_with_options(Some("completed-session"), &FocusOptions::default()) is called
  Then: Returns Error::InvalidSessionStatus

- test_postcondition_zellij_tab_switched_when_inside_zellij
  Given: Session "test-session" exists with status Active
  And: ZELLIJ_SESSION_NAME environment variable is set
  When: run_with_options(Some("test-session"), &FocusOptions::default()) succeeds
  Then: Zellij command to switch tab is executed

- test_postcondition_zellij_attaches_when_outside_zellij
  Given: Session "test-session" exists with status Active
  And: ZELLIJ_SESSION_NAME environment variable is not set
  When: run_with_options(Some("test-session"), &FocusOptions::default()) succeeds
  Then: Zellij attach command is executed

- test_postcondition_session_output_returned_as_json
  Given: Session "test-session" exists with status Active
  When: run_with_options(Some("test-session"), &FocusOptions::default()) succeeds
  Then: Output contains valid JSON with session details

- test_postcondition_action_emitted
  Given: Session "test-session" exists with status Active
  When: run_with_options(Some("test-session"), &FocusOptions::default()) succeeds
  Then: Action with verb "focus", target "test-session", status "completed" is emitted

## Contract Violation Tests

- `test_p1_violation_empty_name_returns_session_name_required`
  Given: No session name is provided
  When: run_with_options(Some(""), &FocusOptions::default()) is called
  Then: Returns `Err(Error::SessionNameRequired)` -- NOT a panic

- `test_p1_violation_whitespace_name_returns_session_name_required`
  Given: Whitespace-only name is provided
  When: run_with_options(Some("   "), &FocusOptions::default()) is called
  Then: Returns `Err(Error::SessionNameRequired)` -- NOT a panic

- `test_p2_violation_nonexistent_session_returns_not_found`
  Given: No session exists
  When: run_with_options(Some("nonexistent"), &FocusOptions::default()) is called
  Then: Returns `Err(Error::SessionNotFound)` -- NOT a panic

- `test_p3_violation_completed_status_returns_invalid_session_status`
  Given: A session exists with status Completed
  When: run_with_options(Some("completed-session"), &FocusOptions::default()) is called
  Then: Returns `Err(Error::InvalidSessionStatus)` -- NOT a panic

- `test_q1_violation_zellij_tab_switch_fails_returns_zellij_command_failed`
  Given: Inside Zellij, but zellij command fails
  When: run_with_options(Some("test-session"), &FocusOptions::default()) is called
  Then: Returns `Err(Error::ZellijCommandFailed)` -- NOT a panic

- `test_q2_violation_zellij_attach_fails_returns_zellij_command_failed`
  Given: Outside Zellij, but zellij attach command fails
  When: run_with_options(Some("test-session"), &FocusOptions::default()) is called
  Then: Returns `Err(Error::ZellijCommandFailed)` -- NOT a panic

## Given-When-Then Scenarios

### Scenario 1: Focus switches to session (lines 103-108)
Given: A session named "feature-focus" exists with status "active"
...
### Scenario 2: Focus from outside Zellij attaches (lines 110-115)
Given: A session named "feature-attach" exists with status "active"
...
### Scenario 3: Focus non-existent session fails (lines 117-120)
Given: No session named "missing" exists
When: I attempt to focus on the session "missing"
Then:
...
- test_doctor_detects_missing_zellij
  Given: zellij is not installed
  When: I run the doctor command
  Then:
    - The "Zellij Installation" check should warn
    - The output should contain suggestion "Install Zellij"
    - The exit code should be 0 (warnings don't cause failure)
