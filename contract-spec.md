...
- **Domain terms**:
  - Session: A isolated workspace with a name, status, and Zellij tab
  - Focus: Switch to a session's Zellij tab (inside Zellij) or attach to session (outside Zellij)
  - Active session: A session with status "active" that can be focused
  - Zellij tab: A tab in Zellij session manager named "isolate:{session_name}"
- **Assumptions**:
  - The implementation will check whether the caller is inside a Zellij session using an environment variable
  - The session must exist in the database before focusing
  - Session status must be "active" to be focusable
  - Zellij command execution is available on the system
- **Open questions**:
  - How exactly do we detect if we are "inside Zellij"? Is there a ZELLIJ_SESSION_NAME environment variable?
  - What is the exact Zellij command to switch tabs vs attach?
