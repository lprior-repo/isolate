...
#BeadImplementation: {
  ...
  contracts_verified: {
    ...
    precondition_checks: [
      "StackMergeState enum exists",
      "merge_queue table exists",
    ]

    // Specific postconditions that must be verified
    postcondition_checks: [
      "stack_merge_state column exists",
      "QueueEntry has stack_merge_state field with sqlx try_from",
    ]

    // Specific invariants that must be maintained
    invariant_checks: [
      "Invalid DB values default to BlockedByParent",
    ]
  }
  ...
  tests_passing: {
    ...
    required_happy_tests: [
      "Column stores state string",
      "Field deserializes correctly",
      "Default applied for NULL",
    ]

    // Required error path tests
    required_error_tests: [
      "Invalid string uses default",
      "No panic on bad data",
    ]
  }
