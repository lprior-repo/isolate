...
#BeadImplementation: {
  ...
  contracts_verified: {
    ...
    precondition_checks: [
      "Stack query methods exist",
      "CLI command pattern exists",
    ]

    // Specific postconditions that must be verified
    postcondition_checks: [
      "stack status command registered",
      "JSONL output with stack info",
    ]

    // Specific invariants that must be maintained
    invariant_checks: [
      "Exit code 0 for found, 1 for not found",
    ]
  }
  ...
  tests_passing: {
    ...
    required_happy_tests: [
      ...
      "Shows depth and root",
    ]

    // Required error path tests
    required_error_tests: [
      "Not in queue shows message",
      "Database error returns proper exit code",
    ]
  }
